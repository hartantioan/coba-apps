...
class MarketingOrderHandoverReceiptController extends Controller
{
   ...
   public function getCode(Request $request){
        ...
        $code = MarketingOrderHandoverReceipt::generateCode($request->val);

		return response()->json($code);
    }

    public function getMarketingReceipt(Request $request){
        $data = MarketingOrderReceipt::whereIn('status',['2','3'])
                ->whereRaw("SUBSTRING(code,8,2) IN ('".implode("','",$this->dataplacecode)."')")
                ->where('grandtotal','>=',0)
                ->where(function($query)use($request){
                    $query->whereDoesntHave('marketingOrderHandoverReceiptDetail')
                        ->orWhereHas('marketingOrderHandoverReceiptDetail',function($query){
    ...
    public function datatable(Request $request){
        ...
        $total_data = MarketingOrderHandoverReceipt::whereRaw("SUBSTRING(code,8,2) IN ('".implode("','",$this->dataplacecode)."')")->count();

        $query_data = MarketingOrderHandoverReceipt::where(function($query) use ($search, $request) {
                if($search) {
        ...
        $total_filtered = MarketingOrderHandoverReceipt::where(function($query) use ($search, $request) {
                if($search) {
                    $query->where(function($query) use ($search, $request) {
                        $query->where('code', 'like', "%$search%")
                            ->orWhere('note', 'like', "%$search%")
            ...
            foreach($query_data as $val) {
                ...
                $response['data'][] = [
                    ...
                    '
                        ...
                        <a href="'.request()->segment(3).'/update_document/'.CustomHelper::encrypt($val->code).'?u='.CustomHelper::encrypt($val->account->employee_no).'&c='.CustomHelper::encrypt($val->account->name).'" class="btn-floating btn-small mb-1 btn-flat waves-effect waves-light indigo accent-1 white-text" data-popup="tooltip" title="Update Status Dokumen" target="_blank"><i class="material-icons dp48">streetview</i></a>
    ...
    public function create(Request $request){
        ...
        $validation = Validator::make($request->all(), [
           ...
           /*  'code'			            => $request->temp ? ['required', Rule::unique('marketing_order_handover_receipts', 'code')->ignore(CustomHelper::decrypt($request->temp),'code')] : 'required|string|min:18|unique:marketing_order_handover_receipts,code',
            'code_place_id'             => 'required', */
            'company_id'			    => 'required',
            'account_id'			    => 'required',
            'post_date'		            => 'required',
        ...
        try {
            ...
            if($validation->fails()) {
                ...
                foreach($request->arr_id as $row){
                    $mohr = MarketingOrderReceipt::where('code',CustomHelper::decrypt($row))->first();
                    $grandtotal += $mohr->grandtotal;
                    $arr_id[] = $mohr->id;
                }

                if($request->temp){
                    $query = MarketingOrderHandoverReceipt::where('code',CustomHelper::decrypt($request->temp))->first();
                    ...
                    if($approved && !$revised){
                        return response()->json([
                            'status'  => 500,
                            'message' => 'Tanda Terima Kwitansi telah diapprove, anda tidak bisa melakukan perubahan.'
                        ]);
                    }
                    ...
                    if(in_array($query->status,['1','6'])){
                        ...
                        if($request->has('document')) {
                            ...
                            $document = $request->file('document')->store('public/marketing_order_handover_receipts');
                        } else {
                        ...
                        foreach($query->marketingOrderHandoverReceiptDetail as $row){
                            $row->delete();
                        }
                    }else{
                        return response()->json([
                            'status'  => 500,
                            'message' => 'Status Tanda Terima Kwitansi sudah diupdate dari menunggu, anda tidak bisa melakukan perubahan.'
                        ]);
                    }
                }else{
                    $lastSegment = $request->lastsegment;
                    $menu = Menu::where('url', $lastSegment)->first();
                    $newCode=MarketingOrderHandoverReceipt::generateCode($menu->document_code.date('y',strtotime($request->post_date)).$request->code_place_id);

                    $query = MarketingOrderHandoverReceipt::create([
                        'code'			            =>  $newCode,
                        'user_id'		            => session('bo_id'),
                        'company_id'                => $request->company_id,
                        'account_id'                => $request->account_id,
                        'post_date'                 => $request->post_date,
                        'document'                  => $request->file('document') ? $request->file('document')->store('public/marketing_order_handover_receipts') : NULL,
                        'note'                      => $request->note,
                        'status'                    => '1',
                ...
                if($query) {
                    ...
                    foreach($arr_id as $key => $row){
                        MarketingOrderHandoverReceiptDetail::create([
                            'marketing_order_handover_receipt_id'   => $query->id,
                            'marketing_order_receipt_id'            => $row,
                        ]);
                    }

                    CustomHelper::sendApproval($query->getTable(),$query->id,$query->note);
                    CustomHelper::sendNotification($query->getTable(),$query->id,'Pengajuan Tanda Terima Kwitansi No. '.$query->code,$query->note,session('bo_id'));

                    activity()
                        ->performedOn(new MarketingOrderHandoverReceipt())
                        ->causedBy(session('bo_id'))
                        ->withProperties($query)
                        ->log('Add / edit tanda terima kwitansi.');
    ...
    public function show(Request $request){
        $po = MarketingOrderHandoverReceipt::where('code',CustomHelper::decrypt($request->id))->first();
        ...
        foreach($po->marketingOrderHandoverReceiptDetail as $row){
            $arr[] = [
    ...
    public function approval(Request $request,$id){
        ...
        $mod = MarketingOrderHandoverReceipt::where('code',CustomHelper::decrypt($id))->first();
        ...
        if($mod){
            ...
            return view('admin.approval.marketing_order_handover_receipt', $data);
        }else{
            abort(404);
        }
    }
    ...
    public function print(Request $request){
        ...
        if($validation->fails()) {
            ...
            foreach($request->arr_id as $key => $row){
                $pr = MarketingOrderHandoverReceipt::where('code',$row)->first();
            ...
            if($validation->fails()) {
                ...
                elseif($total_pdf>31){
                    ...
                    for ($nomor = intval($request->range_start); $nomor <= intval($request->range_end); $nomor++) {
                        ...
                        $query = MarketingOrderHandoverReceipt::where('Code', 'LIKE', '%'.$x)->first();
                        if($query){
                            $pdf = PrintHelper::print($query,'Tanda Terima Kwitansi','a4','portrait','admin.print.sales.order_handover_receipt_individual');
                            $font = $pdf->getFontMetrics()->get_font("helvetica", "bold");
                            $content = $pdf->download()->getOriginalContent();
                            $temp_pdf[]=$content;
                        }
            ...
            if($validation->fails()) {
                ...
                if(count($merged)>31){
                    ...
                    foreach($merged as $code){
                        $query = MarketingOrderHandoverReceipt::where('Code', 'LIKE', '%'.$code)->first();
                        if($query){
                            $pdf = PrintHelper::print($query,'Tanda Terima Kwitansi','a4','portrait','admin.print.sales.order_handover_receipt_individual');
                            $font = $pdf->getFontMetrics()->get_font("helvetica", "bold");
                            $content = $pdf->download()->getOriginalContent();
    ...
    public function voidStatus(Request $request){
        $query = MarketingOrderHandoverReceipt::where('code',CustomHelper::decrypt($request->id))->first();
        ...
        if($query) {
            if(in_array($query->status,['4','5'])){
                ...
                activity()
                    ...
                    ->causedBy(session('bo_id'))
                    ->withProperties($query)
                    ->log('Void the marketing order handover receipt data');

                CustomHelper::sendNotification($query->getTable(),$query->id,'Tanda Terima Kwitansi No. '.$query->code.' telah ditutup dengan alasan '.$request->msg.'.',$request->msg,$query->user_id);
    ...
    public function destroy(Request $request){
        ...
        if($query->delete()) {
            ...
            $query->marketingOrderReceiptDetail()->delete();
            ...
            activity()
                ...
                ->causedBy(session('bo_id'))
                ->withProperties($query)
                ->log('Delete the marketing order receipt data');
    ...
    public function viewStructureTree(Request $request){
        ...
        if($query){
            $data_mo_delivery = [
                ...
                'url'=>request()->root()."/admin/sales/marketing_order_handover_invoice?code=".CustomHelper::encrypt($query->code),
            ];

            $data_go_chart[]= $data_mo_delivery;
            $result = TreeHelper::treeLoop1($data_go_chart,$data_link,'data_id_hand_over_receipt',$query->id);
            $array1 = $result[0];
            $array2 = $result[1];
    ...
    public function simpleStructrueTree(Request $request){
        ...
        if($query){
            ...
            $data_go_chart[]= $data_mo_delivery;
            $result = TreeHelper::simpleTree($data_go_chart,$data_link,'data_id_hand_over_receipt',$query->id);
            $array1 = $result[0];
            $array2 = $result[1];
            $data_go_chart = $array1;
            $data_link = $array2;
    ...
    public function courierIndex(Request $request,$code)
    {
        $mohr = MarketingOrderHandoverReceipt::where('code',CustomHelper::decrypt($code))->whereIn('status',['2','3'])->first();
        if($mohr){
            $data = [
                'title'         => 'Tanda Terima Kwitansi',
                'content'       => 'admin.sales.handover_receipt_update',
                'data'          => $mohr,
                'code'          => $code,
                'account'       => $request->u,
                'courier'       => $request->c,
    ...
    public function courierUpdate(Request $request){
        ...
        try {
            ...
            $query = MarketingOrderHandoverReceipt::where('code',CustomHelper::decrypt($request->code))->whereIn('status',['2','3'])->first();
            ...
            if($query) {
                ...
                $arr = [];
                foreach($request->arr_receipt as $key => $row){
                    $detail = MarketingOrderHandoverReceiptDetail::find(intval($row));
                    $detail->status = $request->arr_status[$key];
                    $detail->note = $request->arr_note[$key];
                    $detail->save();
                    $arr[] = $detail;
                }

                CustomHelper::sendNotificationWithFrom($query->getTable(),$query->id,'Tanda Terima Kwitansi No. '.$query->code.' telah diupdate.','Pengajuan Tanda Terima Kwitansi telah diupdate oleh kurir.',$query->account_id,$query->account_id);

                activity()
                    ->performedOn(new MarketingOrderHandoverReceipt())
                    ->causedBy(session('bo_id'))
                    ->withProperties($arr)
                    ->log('Update kurir tanda terima kwitansi.');

                $response = [
                    'status'    => 200,
                    'message'   => 'Data berhasil disimpan. Halaman akan direfresh.',
                ];
            } else {
                $response = [
                    'status'  => 500,
                    'message' => 'Data belum siap / disetujui.'
                ];
            }
    ...
    public function done(Request $request){
        $query_done = MarketingOrderHandoverReceipt::where('code',CustomHelper::decrypt($request->id))->first();
        ...
        if($query_done){
            ...
            if(in_array($query_done->status,['1','2'])){
                ...
                activity()
                        ...
                        ->log('Done the Marketing Order Handover Receipt data');
