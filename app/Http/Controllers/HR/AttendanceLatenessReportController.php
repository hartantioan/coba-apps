...
class AttendanceLatenessReportController extends Controller
{
    public function index(Request $request)
    {
        $data = [
            ...
            'content'       => 'admin.hr.attendance_lateness_report',
        ];
    ...
    public function filterByDate(Request $request){
        ...
        $user_data = User::where(function($query) use ( $request) {
                if($request->user_id) {
                    $query->where('employee_no', $request->user_id);
                }
            })->get();
    ...
    //     $start_date = Carbon::parse($request->start_date);
    //     $end_date = Carbon::parse($request->end_date);
    //     $query_data = EmployeeSchedule::where(function($query) use ( $start_date,$request,$end_date) {
    //             if($request->start_date && $request->end_date) {
    //                 $query->whereBetween('date', [$start_date, $end_date]);
    //             }
    //         })
    //         ->get();
    //     $query_data2 = Attendances::where(function($query) use ( $start_date,$request,$end_date) {
    //             if($request->start_date && $request->end_date) {
    //                 $query->whereBetween('date', [$start_date, $end_date->addDay()]);
    //             }
    //         })
    //         ->get();
    ...
    //             foreach($query_data as $row_schedule){
    ...
    //                 }
    //             }
    //             foreach($query_data2 as $row_attendance){
    ...
    //                         $schedule_peruser_by_date=[]; //diloop per user untuk mendapatkan schedule yang dibuat untuk peruser masing masing
    //                         foreach($schedule_otd as $row_schedule_filter){
                                
    //                             if($row_schedule_filter->user_id == $row_user->nik){
    //                                 $schedule_peruser_by_date[]=$row_schedule_filter;
    //                             }
    //                         }
    //                         //kalau ada schedule baru masuk
    //                         if($schedule_peruser_by_date){
    //                             $firstSchedule = reset($schedule_peruser_by_date);
    //                             $latestSchedule = end($schedule_peruser_by_date);
    //                             $login=null;
    //                             $logout=null;
    //                             foreach($attendance_otd as $row_attendance_filter){
    //                                 $exact_in  = 0;
    //                                 $exact_out = 0;
    //                                 if($row_attendance_filter->employee_no == $row_user->nik){
                                        
    //                                     $dateAttd = Carbon::parse($row_attendance_filter->date);
    //                                     $timePart = $dateAttd->format('H:i:s');
    //                                     $min_time_in = $firstSchedule->shift->min_time_in;
    //                                     $time_in = $firstSchedule->shift->time_in;
    //                                     $max_time_out = $latestSchedule->shift->max_time_out;
    //                                     $time_out = $latestSchedule->shift->time_out;
    //                                     if ($timePart >= $min_time_in && $timePart <= $time_in) {
    //                                         $exact_in = 1 ;
    //                                         if(!$login){
    //                                             $login = $timePart;
