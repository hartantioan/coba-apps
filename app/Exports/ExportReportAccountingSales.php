        ...
        foreach ($invoice_detail as $key => $row) {
            ...
            if($row->lookable_type != 'marketing_order_down_payments'){
                ...
                $arr[] = [
                    ...
                    'type' => $row->getItem() ?? $row->marketingOrderInvoice->marketingOrderDeliveryProcess?->marketingOrderDelivery->marketingOrderDeliveryDetail->first()->item->type->name ?? '',
                    'brand' => $row->getItem() ?? $row->marketingOrderInvoice->marketingOrderDeliveryProcess?->marketingOrderDelivery->marketingOrderDeliveryDetail->first()->item->brand->name ?? '',
                    'cat_brand' => $row->getItem() ?? $row->marketingOrderInvoice->marketingOrderDeliveryProcess?->marketingOrderDelivery->marketingOrderDeliveryDetail->first()->item->brand->type() ?? '',
                    'qty' => $row->qty,
