...
class LandedCostController extends Controller
{
   ...
   public function getCode(Request $request){
    ...
    public function show(Request $request){
        ...
        foreach($lc->landedCostDetail()->orderBy('id')->get() as $row){
            $arr[] = [
                'item_id'                   => $row->item_id,
                'item_name'                 => $row->item->code.' - '.$row->item->name,
                'qtyRaw'                    => $row->qty,
    ...
    public function simpleStructrueTree(Request $request){
        ...
        if($query) {
            ...
            $result = TreeHelper::simpleTree($data_go_chart,$data_link,'data_id_lc',$query->id);
            $array1 = $result[0];
            $array2 = $result[1];
            $data_go_chart = $array1;
            $data_link = $array2;
    ...
    public function cancelStatus(Request $request){
        ...
        if($query) {
            ...
            if(in_array($query->status,['4','5','8'])){
                ...
                $response = [
                    'status'  => 500,
                    'message' => 'Data telah digunakan pada A/P Invoice atau LC lainnya.'
                ];
            }else{
