...
class OfficialReport extends Model
{
    ...
    {
        if(count($this->officialReportDetail) > 0){
            $arrDoc = [];
            foreach($this->officialReportDetail as $key => $row){
                if(Storage::exists($row->document)){
                    $arrDoc[] = '<a href="'.asset(Storage::url($row->document)).'" target="_blank">Lampiran '.($key + 1).'</a>';
                }
            }
    ...
    public static function generateCode($prefix)
    {
        ...
        $query = OfficialReport::selectRaw('RIGHT(code, 8) as code')
    ...
    public function officialReportDetail()
    {
        return $this->hasMany('App\Models\OfficialReportDetail');
    }

    public function officialReportApprover()
    {
        return $this->hasMany('App\Models\OfficialReportApprover');
    }
}
