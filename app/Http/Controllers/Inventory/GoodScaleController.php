    ...
    public function datatable(Request $request){
        ...
        if($query_data <> FALSE) {
            ...
            foreach($query_data as $val) {
                ...
                $btnRecreate = $val->type == '2' ? '<button type="button" class="btn-floating mb-1 btn-flat waves-effect waves-light green darken-1 white-tex btn-small" data-popup="tooltip" title="Recreate PO dan Jurnal" onclick="recall(`' . CustomHelper::encrypt($val->code) . '`)"><i class="material-icons dp48">loop</i></button> <button type="button" class="btn-floating mb-1 btn-flat waves-effect waves-light orange darken-1 white-tex btn-small" data-popup="tooltip" title="Cancel Timbang Keluar" onclick="alert(`Sabar, coming soon!`);return false;revertScale(`' . CustomHelper::encrypt($val->code) . '`);"><i class="material-icons dp48">do_not_disturb</i></button>' : '';
                ...
                <button type="button" class="btn-floating mb-1 btn-flat waves-effect waves-light red accent-2 white-text btn-small" data-popup="tooltip" title="Delete" onclick="destroy(`' . CustomHelper::encrypt($val->code) . '`)"><i class="material-icons dp48">delete</i></button> '.$btn_jurnal.' '.$btnRecreate;
                $response['data'][] = [
                    ...
                    CustomHelper::formatConditionalQty($val->qty_sj),
            ...
            DB::beginTransaction();
                ...
                if($request->temp){
                    ...
                    $query = GoodScale::create([
                        ...
                        'qty_sj'                    => str_replace(',','.',str_replace('.','',$request->qty_sj)),
            ...
            if($query_done){
                ...
                if(!$query_done->journal()->exists()){
                    ...
                    if($totalProportional > 0){
                        ...
                        if($gs->type == '2' && $gs->goodScaleDetail()->exists() && $gs->qty_final > 0 && $gs->hasFrancoMod() && !$gs->journal()->exists() && $gs->sjHasReceivedDocument()){
                            ...
                            $query = Journal::create([
                                ...
                                'code'			=> Journal::generateCode('JOEN-'.date('y',strtotime($gs->post_date)).'00'),
    ...
    public function viewJournal(Request $request,$id){
        ...
        if($query->journal()->exists()){
            $response = [
                ...
                'note'      => $query->note,
                'post_date' => date('d/m/Y',strtotime($query->journal->post_date)),
            ];
            $string='';
