   ...
   public function getCode(Request $request){
    ...
    public function getAccountData(Request $request){
        ...
        foreach($data as $d) {
            ...
            $asset[] = [
                ...
                'code'                  => $d->code,
                'name'                  => $d->name,
                'group'                 => $d->assetGroup->name,
                'date'                  => $d->date,
                'nominal'               => $d->nominal > 0 ? number_format($d->nominal,2,',','.') : '0,00',
                'accumulation_total'    => number_format($d->accumulation_total,2,',','.'),
        ...
        if($validation->fails()) {
            ...
            foreach($request->arr_id as $key =>$row){
                ...
                if($pr){
                    ...
                    $pdf = PrintHelper::print($pr,'Capitalization','a5','landscape','admin.print.accounting.capitalization_individual');
            ...
            if($validation->fails()) {
                ...
                elseif($total_pdf>31){
                    ...
                    for ($nomor = intval($request->range_start); $nomor <= intval($request->range_end); $nomor++) {
                        ...
                        if($query){
                            $pdf = PrintHelper::print($query,'Capitalization','a5','landscape','admin.print.accounting.capitalization_individual');
                            $font = $pdf->getFontMetrics()->get_font("helvetica", "bold");
    ...
    public function getAsset(Request $request){
        foreach($request->arr_asset_id as $row){
            ...
            'unit_name'             => $capital ? $capital->unit->name : '',
            'unit_id'               => $capital ? $capital->unit_id : '',
            'nominal'               => $data->nominal > 0 ? number_format($data->nominal,2,',','.') : '0,00',
            'price'                 => $capital ? number_format($capital->price,2,',','.') : '0,00',
            'place_id'              => $data->place_id,
            'place_name'            => $data->place->name,
            'place_code'            => $data->place->code,
            'book_balance'          => $data->book_balance,
            'qty_balance'           => $data->qtyBalance(),
