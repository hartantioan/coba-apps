...
class ExportMarketingRecapitulationCsv implements FromCollection, WithTitle, ShouldAutoSize
{
    ...
    public function collection()
    {
        ...
        foreach ($ardp as $key => $row) {
            ...
            $arr[] = [
                '1'     => 'FK;' . $transactionCode . ';' . $revCode . ';' . $tax_no . ';' . $month . ';' . $year . ';' . $newdate . ';' . $row->getNpwp() . ';' . $row->account->userDataDefault()->title . ';' . $row->account->userDataDefault()->address . ';' . round($row->total, 0) . ';' . round($row->tax, 0) . ';0;;1;' . floor($row->total * (11/12)) . ';' . floor($row->tax) . ';0;' . $row->code . ';;'
            ];
            $arr[] = [
