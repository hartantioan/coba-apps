   ...
   public function getCode(Request $request){
    ...
    public function create(Request $request){
        ...
        if($validation->fails()) {
            ...
            foreach($request->arr_item as $key => $row){
                ...
                $itemGroup_warehouse = $item->itemGroup->itemGroupWarehouse->pluck('warehouse_id')->toArray();
    ...
    public function rowDetail(Request $request){
        ...
        $string = '<div class="row pt-1 pb-1 lighten-4"> <div class="col s12">'.$data->code.$x.'</div>
                    <div class="col s12" style="font-weight:bold;"> Dari: '.$data->warehouseFrom->name.' Ke: '.$data->warehouseTo->name.'</div>
                    <div class="col s12">
    ...
    public function printIndividual(Request $request,$id){
        ...
        $formattedDate = $currentDateTime->format('d/m/Y H:i:s');
        if($pr){
            $pdf = PrintHelper::print($pr,'Inventory Transfer Out','a5','landscape','admin.print.inventory.inventory_transfer_out_individual',$menuUser->mode);
