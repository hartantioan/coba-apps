...
class ProductionRepackController extends Controller
{
        ...
        if($query_data <> FALSE) {
            ...
            foreach($query_data as $val) {
                ...
                $response['data'][] = [
                    ...
                    $val->status(),
                    in_array($val->status,['1','2','3']) ? $val->user->name : $val->voidUser->name,
                    '
    ...
    public function voidStatus(Request $request){
        ...
        if($query) {
            ...
            foreach($query->productionRepackDetail as $row){
                ...
                if($cekStock){
                    if(round($cekStock->qty,3) !== round($row->qty,3)){
                        $stockUsed = true;
                    }
                }
            }
    ...
    public function export(Request $request){
		...
		return Excel::download(new ExportProductionRepacking($post_date,$end_date), 'repack'.uniqid().'.xlsx');
    }
}
