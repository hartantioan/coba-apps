<style>
    ...
    #modal7 input[type=text]:not(.browser-default){
            ...
            <div class="col s12">
                ...
                <form class="row" id="form_data" onsubmit="return false;">
                    ...
                    <div class="col s12">
                        <fieldset>
                            ...
                            <div class="row">
                                ...
                                <div class="input-field col m4 s12 step4">
                                    <input id="due_date" name="due_date" min="{{ date('Y-m-d') }}" type="date" max="{{ date('9999'.'-12-31') }}" placeholder="Tgl. posting">
                        ...
                        <div class="row">
                            <div class="input-field col m4 s12 step13 mt-1">
...
<script>
                    ...
                    $.each(response.details, function(i, val) {
                        ...
                        $('#body-item').append(`
                            <tr class="row_item" data-id="` + val.reference_id + `">
                                ...
                                <td>
                                    <input name="arr_qty[]" onfocus="emptyThis(this);" type="text" id="rowQty` + count + `" value="` + val.qty + `" onkeyup="formatRupiahNoMinus(this);countRow('` + count + `')">
                                </td>
                                <td class="center">
                        ...
                        $.each(datakuy.details, function(i, val) {
                            ...
                            $('#body-item').append(`
                                <tr class="row_item" data-id="` + $('#material_request_id').val() + `">
                                    ...
                                    <td>
                                        <input name="arr_qty[]" onfocus="emptyThis(this);" id="rowQty` + count + `" type="text" value="` + val.qty_balance + `" onkeyup="formatRupiahNoMinus(this);countRow('` + count + `')">
                                    </td>
                                    <td class="center">
                                        <select class="browser-default" id="arr_satuan` + count + `" name="arr_satuan[]" required onchange="countRow('` + count + `')">
                                            <option value="">--Silahkan pilih item--</option>    
                                    ...
                                    <td>
                                        <input name="arr_required_date[]" type="date" max="{{ date('9999'.'-12-31') }}" value="` + val.date + `">
                                    </td>
                                    <td>
                                        <select class="browser-default" id="arr_place` + count + `" name="arr_place[]">
                                            @foreach ($place as $rowplace)
    ...
    function useStock(){
        ...
        $.map(table_multi.rows('.selected').nodes(), function (item) {
            ...
            arr_name.push($(item).data('itemname'));
            arr_qty.push($('#arr_qty_order' + $(item).data('code')).val());
            ...
            for(let i = 0; i < arr_id.length;i++){
                ...
                $('#body-item').append(`
                    <tr class="row_item">
                        ...
                        <td>
                            <input name="arr_qty[]" onfocus="emptyThis(this);" type="text" value="` + arr_qty[i] + `" onkeyup="formatRupiahNoMinus(this);countRow('` + count + `')" data-stockqty="` + arr_qty[i] + `" id="rowQty` + count + `">
                        </td>
                        <td class="center">
                ...
                `);
                $("#arr_warehouse" + count).empty();
                if(arr_warehouse[i].length > 0){
                    $.each(arr_warehouse[i], function(i, value) {
                        $('#arr_warehouse' + count).append(`
                            <option value="` + value.id + `">` + value.name + `</option>
                        `);
                    });
                ...
                $("#arr_satuan" + count).empty();
                if(arr_unit[i].length > 0){
                    $.each(arr_unit[i], function(i, value) {
                        $('#arr_satuan' + count).append(`
                            <option value="` + value.id + `" data-conversion="` + value.conversion + `">` + value.code + `</option>
                        `);
                    });
                        ...
                        $.each(response.details, function(i, val) {
                            ...
                            $('#body-stock').append(`
                                <tr class="row_stock" data-item="` + val.item_id + `" data-uom="` + val.uom_unit + `" data-warehouse='` + JSON.stringify(val.list_warehouse) + `' data-unit='` + JSON.stringify(val.buy_units) + `' data-itemname="` + val.item_name + `" data-itemunit="`+ val.unit +`" data-code="` + count + `" data-place="` + val.place_id + `">
                                    ...
                                    <td class="right-align">
                                        <input name="arr_qty_order[]" onfocus="emptyThis(this);" type="text" value="` + val.qty_request + `" onkeyup="formatRupiahNoMinus(this)" id="arr_qty_order` + count + `" style="text-align:right;width:100px;">
                                    </td>
                                </tr>
                            `);
                        });
