...
class MarketingOrderDeliveryController extends Controller
{
   ...
   public function getCode(Request $request){
            ...
            if($validation->fails()) {
                ...
                if($request->arr_item_shading_id){
                    ...
                    foreach($arrShadingItem as $key => $row){
                        ...
                        if($itemShading){
                            ...
                            info($stock.' - '.round($arrShadingQty[$key],3));
                ...
                if($request->tempRevision){
                    ...
                    if($queryrevision){
                        ...
                        return response()->json([
                            'status'  => 500,
                            'message' => 'Untuk mode REMAP, MOD yang diremap haruslah MOD status PROSES saja.',
                        ]);
                    }
                }
    ...
    public function show(Request $request){
        ...
        if(!$request->revision){
            if($po->status !== '2'/*  || $po->stage_status == '2' */ || $po->marketingOrderDeliveryProcess()->exists()){
                return response()->json([
                    'status'  => 500,
            ...
            if(!$po->goodScaleDetail()->exists()){
                ...
                if(!$po->goodScaleDetail->goodScale->time_scale_out || !$po->goodScaleDetail->goodScale->time_scale_in){
                    return response()->json([
                        'status'  => 500,
                        'message' => 'Mohon maaf, data timbangan harus diisi timbang keluar dan timbang masuknya.',
                    ]);
                }
            }
