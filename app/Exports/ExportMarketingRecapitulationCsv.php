    ...
    */
    ...
    public function collection()
    {
        ...
        foreach($ardp as $key => $row){
            ...
            $arr[] = [
                '1'     => 'FK;'.$transactionCode.';0;'.$tax_no.';'.$month.';'.$year.';'.$newdate.';'.$row->account->userDataDefault()->npwp.';'.$row->account->userDataDefault()->title.';'.$row->account->userDataDefault()->address.';'.round($row->total,0).';'.round($row->tax,0).';0;;2;0;0;0;'.$row->code.';;'
            ];
            $arr[] = [
