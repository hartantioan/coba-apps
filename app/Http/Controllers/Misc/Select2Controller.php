...
class Select2Controller extends Controller {
    ...
    public function simpleItem(Request $request)
    {
        $response = [];
        $search   = $request->search;
        $data = Item::where(function($query) use($search){
    ...
    public function itemForWeight(Request $request)
    {
        $response = [];
        $search   = $request->search;
        $data = Item::where(function($query) use($search){
    ...
    public function customer(Request $request)
    {
        ...
        foreach($data as $d) {
            $response[] = [
                ...
                'deposit'               => number_format($d->deposit,2,',','.') ?? 0,
    ...
    public function supplierVendorCustomer(Request $request)
    {
        $response = [];
        $search   = $request->search;
        $data = User::where(function($query) use($search){
    ...
    public function pallet(Request $request)
    {
        ...
        $data = Pallet::where(function($query) use($search){
                ...
                ->where(function($query)use($request){
                    if($request->arr_pallet){
                        $query->whereIn('id',$request->arr_pallet);
                    }
                })
                ->where('status','1')->get();
    ...
    public function issueGlaze(Request $request)
    {
        ...
        ->where(function($query) use($request,$itemtarget){
            if($request->arrissue){
    ...
    public function marketingOrderByAccount(Request $request)
    {
        ...
        ->where(function($query)use($request){
            ...
            if($request->so_type){
                $query->where('type',floatval($request->so_type));
            }
        })
        ->whereDoesntHave('used')
        ->whereRaw("SUBSTRING(code,8,2) IN ('".implode("','",$this->dataplacecode)."')")
    ...
    public function marketingOrderDelivery(Request $request)
    {
        ...
        ->where(function($query){
            $query->where(function($query){
                $query->whereNotNull('marketing_order_delivery_id');
            })->orWhere(function($query){
    ...
    public function productionBatch(Request $request)
    {
        ...
        foreach($data as $d) {
            $response[] = [
                'id'   			=> $d->id,
                'text' 			=> $d->code.' - Tgl. '.$d->getDate().' Qty '.CustomHelper::formatConditionalQty($d->qty).' '.$d->item->uomUnit->code,
                'code'          => $d->code,
    ...
    public function marketingOrderDeliveryProcessComplaint(Request $request)
    {
        $response = [];
        $search     = $request->search;
        $account_id = $request->account_id;
    ...
    public function sampleTestInputProc(Request $request)
    {
        $response = [];
        $search   = $request->search;
        $data = SampleTestInput::where(function($query) use($search,$request){
    ...
    public function sampleTestInputQc(Request $request)
    {
        $response = [];
        $search   = $request->search;
        $data = SampleTestInput::where(function($query) use($search,$request){
