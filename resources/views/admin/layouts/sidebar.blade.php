    ...
    <aside class="sidenav-main nav-expanded nav-lock nav-collapsible sidenav-light sidenav-active-square">
        ...
        <ul class="sidenav sidenav-collapsible leftside-navigation collapsible sidenav-fixed menu-shadow" id="slide-out" data-menu="menu-navigation" data-collapsible="menu-accordion">
			<li class="{{ Request::segment(2) == 'dashboard' ? 'active' : '' }} bold"><a class="waves-effect waves-cyan {{ Request::segment(2) == 'dashboard' ? 'active' : '' }}" href="{{ url('admin/dashboard') }}"><i class="material-icons">settings_input_svideo</i><span class="menu-title" data-i18n="Dashboard">Dashboard</span></a>

            @php $menu = App\Models\Menu::whereNull('parent_id')->where('status','1')->oldest('order')->get(); @endphp

            @foreach($menu as $m)
                @if($m->sub()->exists())
                    <li class="{{ Request::segment(2) == $m->url ? 'active open' : '' }} bold"><a class="collapsible-header waves-effect waves-cyan " href="JavaScript:void(0)"><i class="material-icons">{{ $m->icon }}</i><span class="menu-title" data-i18n="{{ $m->name }}">{{ $m->name }}</span></a>
                        <div class="collapsible-body">
                            <ul class="collapsible collapsible-sub" data-collapsible="accordion">
                                @foreach($m->sub()->where('status','1')->oldest('order')->get() as $msub)
                                    @if($msub->sub()->exists())
                                        <li class="{{ Request::segment(3) == $msub->url ? 'active open' : '' }} bold menu-child1"><a class="collapsible-header waves-effect waves-cyan " href="JavaScript:void(0)"><i class="material-icons">{{ $msub->icon }}</i><span class="menu-title" data-i18n="{{ $msub->name }}">{{ $msub->name }}</span></a>
                                            <div class="collapsible-body">
                                                <ul class="collapsible collapsible-sub" data-collapsible="accordion">
