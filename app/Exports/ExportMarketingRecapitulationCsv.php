...
class ExportMarketingRecapitulationCsv implements FromCollection, WithTitle, ShouldAutoSize
{
    ...
    public function collection()
    {
        ...
        foreach ($invoice as $key => $row) {
            ...
            if ($row->total > 0) {
                $arr[] = [
                    '1'     => 'FK;' . $transactionCode . ';0;' . $tax_no . ';' . $month . ';' . $year . ';' . $newdate . ';' . $row->getNpwp() . ';' . $row->userData->title . ';' . $row->userData->address . ';' . floor($row->total) . ';' . floor($row->tax) . ';0;'.$freeAreaTax.';0;0;0;0;' . $row->code. ($row->no_pjb ? '/PPBJ : '.$row->no_pjb : '') . ';;'
                ];
            } else {
                $arr[] = [
                    '1'     => 'FK;' . $transactionCode . ';0;' . $tax_no . ';' . $month . ';' . $year . ';' . $newdate . ';' . $row->getNpwp() . ';' . $row->userData->title . ';' . $row->userData->address . ';' . floor($row->subtotal) . ';' . floor($row->subtotal*($row->taxMaster->percentage/100)) . ';0;'.$freeAreaTax.';2;0;0;0;' . $row->code. ($row->no_pjb ? '/PPBJ : '.$row->no_pjb : '') . ';;'
                ];
            }
