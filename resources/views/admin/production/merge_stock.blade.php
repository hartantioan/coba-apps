                                            ...
                                            <table id="datatable_serverside" >
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th>Target Item</th>
                                                        <th>Target Shading</th>
                                                        <th>Batch</th>
                                                        <th>Qty Hasil</th>
                                                        <th>Satuan</th>
                                                        <th style="@if(app()->getLocale() == 'chi') font-weight:normal !important;@endif">{{ __('translations.plant') }}</th>
                                                        <th style="@if(app()->getLocale() == 'chi') font-weight:normal !important;@endif">{{ __('translations.warehouse') }}</th>
                                                        <th>Area</th>
                            ...
                            <div class="col s12">
                                <fieldset>
                                    ...
                                    <div class="input-field col m3 s12">
                                        ...
                                        <label class="" for="place_id">Lokasi Stock</label>
                                    </div>
                                    <div class="input-field col m3 s12">
                                        ...
                                        <label class="" for="area_id">Area Tujuan</label>
                                    </div>
                                    <div class="input-field col m3 s12">
                                        <select class="browser-default" id="item_shading_id" name="item_shading_id" onchange="resetDetail();"></select>
                                        <label class="active" for="item_shading_id">Item Target & Shading</label>
                                    </div>
                        ...
                        <div class="row">
                            <div class="col s12 step9">
                                <fieldset style="min-width: 100%;">
                                    <legend>2. Detail Item Issue</legend>
                                    <div class="col m12 s12">
                                        <div class="card-alert card red">
                                            <div class="card-content white-text">
                                                <p>INFO PENTING : Item yang bisa digabungkan menjadi 1 adalah item dengan 1 shading, dan akan menerbitkan kode batch produksi baru.</p>
                                            </div>
                                        </div>
                                        <div class="col s12" style="min-width:100%;max-width:100%">
                                            <p class="mt-2 mb-2">
                                                <table class="bordered" style="border: 1px solid;" id="table-detail-item-issue">
                                                    <thead>
                                                        <tr>
                                                            <th class="center" width="5%">{{ __('translations.delete') }}</th>
                                                            <th class="center" width="5%">{{ __('translations.no') }}.</th>
                                                            <th class="center" width="50%">Stock</th>
                                                            <th class="center" width="10%">Qty</th>
                                                            <th class="center" width="5%">Satuan</th>
                                                        </tr>
                                                    </thead>
                                                    ...
                                                    <tfoot>
                                                        <tr>
                                                            ...
                                                            <th class="right-align" id="total-qty-issue">
                                                                0,000
                                                            </th>
                                                            <th>
...
<script>
    ...
    $(function() {
        ...
        $('#item_shading_id').select2({
            ...
            ajax: {
                url: '{{ url("admin/select2/sales_item_shading_box") }}',
                type: 'GET',
                dataType: 'JSON',
                delay: 250,
    ...
    function applyMax(code){
        $('#rowQty' + code).data('max','0');
        $('#arr_unit' + code).text('-');
        if($('#arr_item_stock' + code).val()){
            $('#rowQty' + code).data('max',$("#arr_item_stock" + code).select2('data')[0].qty);
            $('#arr_unit' + code).text($("#arr_item_stock" + code).select2('data')[0].unit);
        }else{
            $('#rowQty' + code).val('0,000');
        }
        countQty();
    }
    ...
    function setArray(){
        ...
        $('*[name^="arr_item_stock[]"]').each(function(index){
            if($(this).val()){
                arr_item_shading.push($(this).val());
            }
        });
    }

    function resetDetail(){
        if($('#item_shading_id').val()){
    ...
    function addItem(){
        if($('#last-row-item-issue').length > 0){
        ...
        if($('#item_shading_id').val()){
            ...
            $('#body-item-issue').append(`
                <tr class="row_item_issue">
                    ...
                    <td>
                        <select class="browser-default" id="arr_item_stock` + count + `" name="arr_item_stock[]" onchange="applyMax('` + count + `');setArray();"></select>
                    </td>
                    <td class="center">
                        <input name="arr_qty[]" onfocus="emptyThis(this);" type="text" value="0,000" onkeyup="formatRupiahNoMinus(this);countQty();" style="text-align:right;width:100%;" id="rowQty`+ count +`" required data-id="` + count + `" data-max="0">
                    </td>
            ...
            $('#arr_item_stock' + count).select2({
                ...
                dropdownParent: $('body').parent(),
                ajax: {
                    url: '{{ url("admin/select2/item_stock_by_shading_place") }}',
                    type: 'GET',
                    dataType: 'JSON',
                    delay: 250,
                    data: function(params) {
                        return {
                            search: params.term,
                            item_shading_id: $('#item_shading_id').val(),
                            place_id: $('#place_id').val(),
                            arr_shading: arr_item_shading,
                        };
                    },
                    processResults: function(data, params) {
                        params.page = params.page || 1;
    ...
    function countQty(){
        ...
        $('*[name^="arr_qty[]"]').each(function(index){
            ...
            if(qty > max){
                $(this).val(
                    formatRupiahIni(max.toFixed(3).toString().replace('.',','))
                );
            }
        });
     ...
     function loadDataTable() {
		window.table = $('#datatable_serverside').DataTable({
            ...
            columns: [
                ...
                { name: 'item_shading_id', className: '' },
                { name: 'batch_no', className: '' },
                { name: 'qty', className: 'right-align' },
                { name: 'unit', searchable: false, orderable: false, className: 'center-align' },
                { name: 'to_plant_id', searchable: false, orderable: false, className: 'center-align' },
                { name: 'to_warehouse_id', searchable: false, orderable: false, className: 'center-align' },
                { name: 'to_area_id', searchable: false, orderable: false, className: 'center-align' },
		...
		swal({
            ...
            if (willDelete) {
                ...
                $('*[name^="arr_item_stock[]"]').each(function(index){
                    ...
                    if(!$('*[name^="arr_qty[]"]').eq(index).val()){
                        passedInput = false;
                    }else{
                        if(parseFloat($('*[name^="arr_qty[]"]').eq(index).val().replaceAll(".", "").replaceAll(",",".")) <= 0){
                            passedInput = false;
                        }
                    }
                });
                ...
                if(!passedInput){
                    swal({
                        title: 'Ups! Maaf.',
                        text: 'Stock dan qty harus diisi.',
                        icon: 'error'
                    });
                }else{
                    var path = window.location.pathname;
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if(response.status == 200){
                    ...
                    $('#place_id').val(response.data.to_place_id).formSelect();
                    $('#area_id').val(response.data.to_area_id).formSelect();
                    $('#company_id').val(response.data.company_id).formSelect();
                    $('#item_shading_id').empty().append(`
                        <option value="` + response.data.item_shading_id + `">` + response.data.item_shading_code + `</option>
                    `);
                    ...
                    $.each(response.data.details, function(i, val) {
                        ...
                        $('#body-item-issue').append(`
                            <tr class="row_item_issue">
                                ...
                                <td class="center">
                                    <input name="arr_qty[]" onfocus="emptyThis(this);" type="text" value="` + val.qty + `" onkeyup="formatRupiahNoMinus(this);countQty();" style="text-align:right;width:100%;" id="rowQty`+ count +`" required data-id="` + count + `" data-max="` + val.max + `">
                                </td>
                                <td class="center" id="arr_unit` + count + `">
                                    ` + val.uom + `
                                </td>
                        ...
                        $('#arr_item_stock' + count).append(`
                            <option value="` + val.item_stock_id + `">` + val.item_stock_note + `</option>
                        `);
