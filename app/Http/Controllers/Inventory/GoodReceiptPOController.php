...
class GoodReceiptPOController extends Controller
{
        ...
        if($validation->fails()) {
            ...
            foreach($request->arr_id as $key =>$row){
                ...
                if($pr){
                    ...
                    $data["image"]=$path_img;
                    $pdf = Pdf::loadView('admin.print.inventory.good_receipt_individual', $data)->setPaper('a5', 'landscape');
                    $pdf->render();
    ...
    public function exportFromTransactionPage(Request $request){
		...
		return Excel::download(new ExportGoodReceiptTransactionPage($search,$post_date,$end_date,$status,$modedata), 'purchase_request_'.uniqid().'.xlsx');
    }
        ...
        if($query) {
            ...
            foreach($query->goodReceiptDetail as $good_receipt_detail){
                ...
                if($purchase_order_detail->purchaseRequestDetail()->exists()){
                    ...
                    $data_id_pr[]=$purchase_request_detail->purchaseRequest->id;
                    $data_go_chart[]=$pr;
                    $data_link[]=[
                        'from'=>$pr["key"],
                        'to'=>$data_po["key"],
                        'string_link'=>$pr["key"].$data_po["key"],
                    ];  
                ...
                $data_link[]=[
                    'from'=>$data_po["key"],
                    'to'=>$query->code,
                    'string_link'=>$data_po["key"].$query->code,
                ];  
