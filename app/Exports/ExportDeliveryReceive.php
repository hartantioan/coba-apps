...
class ExportDeliveryReceive implements FromCollection, WithTitle, WithHeadings, ShouldAutoSize
{
    ...
    {
        $query_data = DeliveryReceiveDetail::whereHas('deliveryReceive', function ($query) {
                if($this->search) {
        ...
        foreach($query_data as $key => $row){
            ...
            $arr[] = [
                'no'         => $key + 1,
                'status'     => $row->deliveryReceive->statusRaw(),
                'kode'       => $row->deliveryReceive->code,
                'user'       => $row->deliveryReceive->user->name ?? '',
                'tgl_posting'=> $row->deliveryReceive->post_date ? date('d/m/Y', strtotime($row->deliveryReceive->post_date)) : '',
                'item'       => $row->item->name ?? '',
                'qty'        => number_format($row->qty, 2, ',', '.'), // optional: adjust decimals
                'harga'      => number_format($row->price, 2, ',', '.'),
                'keterangan' => $row->note,
                'supplier'   => $row->deliveryReceive->account->name ?? '',
            ];
