        ...
        if($validation->fails()) {
            ...
            foreach($request->arr_id as $key =>$row){
                ...
                if($pr){
                    ...
                    $data["image"]=$path_img;
                    $pdf = Pdf::loadView('admin.print.accounting.depreciation_individual', $data)->setPaper('a5', 'landscape');
                    $pdf->render();
            ...
            if($validation->fails()) {
                ...
                elseif($total_pdf>31){
                    ...
                    for ($nomor = intval($request->range_start); $nomor <= intval($request->range_end); $nomor++) {
                        $query = Depreciation::where('Code', 'LIKE', '%'.$nomor)->first();
                        if($query){
                            $data = [
                                'title'     => 'Depreciation',
                                'data'      => $query
            ...
            if($validation->fails()) {
                ...
                if(count($merged)>31){
                    ...
                    foreach($merged as $code){
                        $query = Depreciation::where('Code', 'LIKE', '%'.$code)->first();
                        if($query){
                            $data = [
                                'title'     => 'Depreciation',
                                'data'      => $query
                            ...
                            $path_img = 'data:image/' . $extencion . ';base64,' . $img_base_64;
                            $data["image"]=$path_img;
                            $pdf = Pdf::loadView('admin.print.accounting.deprecitation_individual', $data)->setPaper('a5', 'landscape');
                            $pdf->render();
                            $font = $pdf->getFontMetrics()->get_font("helvetica", "bold");
                            $pdf->getCanvas()->page_text(505, 350, "PAGE: {PAGE_NUM} of {PAGE_COUNT}", $font, 10, array(0,0,0));
                            $pdf->getCanvas()->page_text(422, 360, "Print Date ". $formattedDate, $font, 10, array(0,0,0));
