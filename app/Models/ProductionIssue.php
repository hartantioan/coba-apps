...
class ProductionIssue extends Model
{
    ...
    public function hasChildDocument(){
        ...
        if($this->productionReceiveIssue()->whereNull('is_auto')->exists()){
            $hasRelation = true;
        }
    ...
    public function listItemAndQty(){
        $arr = [];

        if($this->productionOrderDetail->productionScheduleDetail->bom->group == '3'){
            foreach($this->productionIssueDetail as $row){
                $arr[] = $row->lookable->name.' - '.CustomHelper::formatConditionalQty($row->qty).' '.$row->lookable->uomUnit->code;
            }
        }
