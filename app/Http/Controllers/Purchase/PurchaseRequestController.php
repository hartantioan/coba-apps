...
class PurchaseRequestController extends Controller
{
    ...
    public function rowDetail(Request $request)
    {
        ...
        if (isset($data->void_date)) {
            $voidUser = $data->voidUser ? $data->voidUser->employee_no . '-' . $data->voidUser->name : 'SYSTEM';
            $x .= '<span style="color: red;">|| Tanggal Void: ' . $data->void_date .  ' || Void User: ' . $voidUser.' || Note:' . $data->void_note.'</span>' ;
        }if($data->status == 3){
            $doneUser = $data->done_id ? $data->doneUser->employee_no . '-' . $data->doneUser->name : 'SYSTEM';
   ...
   public function getCode(Request $request){
    ...
    public function viewStructureTree(Request $request){
        ...
        if($query) {
            $result = TreeHelper::treeLoop1($data_go_chart,$data_link,'data_id_pr',$query->id);
            $array1 = $result[0];
            $array2 = $result[1];
