...
class OutgoingPaymentController extends Controller
{
        ...
        if($validation->fails()) {
            ...
            foreach($request->arr_id as $key =>$row){
                ...
                if($pr){
                    ...
                    $pdf = PrintHelper::print($pr,'Outgoing Payment','A4','portrait','admin.print.finance.outgoing_payment_individual');
            ...
            if($validation->fails()) {
                ...
                elseif($total_pdf>31){
                    ...
                    for ($nomor = intval($request->range_start); $nomor <= intval($request->range_end); $nomor++) {
                        ...
                        if($query){
                            $pdf = PrintHelper::print($query,'Outgoing Payment','A4','portrait','admin.print.finance.outgoing_payment_individual');
                            $font = $pdf->getFontMetrics()->get_font("helvetica", "bold");
    ...
    public function printIndividual(Request $request,$id){
        ...
        $formattedDate = $currentDateTime->format('d/m/Y H:i:s');
        if($pr){
            $pdf = PrintHelper::print($pr,'Outgoing Payment','A4','portrait','admin.print.finance.outgoing_payment_individual',$menuUser->mode);
    ...
    public function export(Request $request){
        ...
        $file_name = 'outgoing_payment'.uniqid().'.xlsx';
        $user_id = session('bo_id');
        ReportFinanceJob::dispatch(\App\Exports\ExportOutgoingPayment::class,$post_date, $end_date, $mode,$user_id,$file_name);
    ...
    public function simpleStructrueTree(Request $request){
        ...
        if($query) {
            ...
            $result = TreeHelper::simpleTree($data_go_chart,$data_link,'data_id_op',$query->id);
            $array1 = $result[0];
            $array2 = $result[1];
            $data_go_chart = $array1;
            $data_link = $array2;
