...
class GoodIssueRequest extends Model
{
    use HasFactory, SoftDeletes, Notifiable;

    protected $table = 'good_issue_requests';
    ...
    {
        return $this->hasMany('App\Models\GoodIssueRequestDetail');
    }
    ...
    public function hasBalanceQtyGi(){
        $status = false;
        foreach($this->goodIssueRequestDetail()->where('status','1')->get() as $row){
            if($row->balanceGi() > 0){
                $status = true;
            }
        }
