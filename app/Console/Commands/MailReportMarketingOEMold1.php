...
class MailReportMarketingOEMold1 extends Command
{
	/**
	 ...
	 * @var string
	 */
	protected $signature = 'emailmarketingoemold:run';
...
END
		...
		$query = DB::select("		   SELECT concat(concat(tipe,' ') ,jenis) AS jenis,brand, SUM(qty) AS endstock FROM (
                ...
                GROUP BY e.name,f.name,g.name
                UNION ALL
                SELECT e.`name` AS tipe, f.name AS brand, g.name AS jenis, coalesce(SUM(b.qty),0)*-1 AS GI
                FROM good_issues a
