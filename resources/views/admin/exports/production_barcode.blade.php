<table border="1" cellpadding="2" cellspacing="0" style="width:100%; font-size:13px;border-collapse: collapse;">
    ...
    <tbody>
        ...
        @endphp
        @foreach($data as $row)
            @foreach ($row->productionBarcodeDetail as $row_detail)
                <tr align="center">
                    <td>{{ $no }}</td>
                    <td>{{ $row->code }}</td>
                    <td>{!! $row->statusRaw() !!}</td>
                    ...
                    <td>
                        @if ($row_detail->productionFgReceiveDetail()->exists())
                            {{ $row_detail->productionFgReceiveDetail->productionFgReceive->productionIssueList() }}
                        @else
                            -
                        @endif
                    </td>
