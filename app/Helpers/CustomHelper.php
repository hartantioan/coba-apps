				...
				if($row->lookable_type == 'coas'){
					...
					if($row->tax_id){
						JournalDetail::create([
							...
							'nominal'		=> $row->tax,
							'nominal_fc'	=> $row->tax,
							'note'			=> $row->purchaseInvoice->tax_no,
							'note2'			=> date('d/m/Y',strtotime($row->purchaseInvoice->cut_date))
						]);
					}
					...
					if($row->wtax_id){
						JournalDetail::create([
							...
							'nominal'		=> $row->wtax,
							'nominal_fc'	=> $row->wtax,
							'note'			=> $row->purchaseInvoice->tax_cut_no,
							'note2'			=> date('d/m/Y',strtotime($row->purchaseInvoice->cut_date))
						]);
					}
					...
					$type = $row->lookable->landedCost->currency->type;
					JournalDetail::create([
						'journal_id'	=> $query->id,
					...
					$type = $row->lookable->purchaseOrderDetail->purchaseOrder->currency->type;
					JournalDetail::create([
						'journal_id'	=> $query->id,
