    ...
    public function collection()
    {
        ...
        $query = DB::select("
            SELECT
                ...
                a.type_brand,
                a.grades,
                COALESCE(b.initialstock, 0) AS initial,
                COALESCE(c.receivefg, 0) AS receivefg,
                COALESCE(d.repackout, 0) AS repackout,
                COALESCE(e.repackin, 0) AS repackin,
                COALESCE(f.gr, 0) AS gr,
                COALESCE(g.gi, 0) AS gi,
                ...
                (
                    ...
                    f.name AS pattern,
                    g.name AS type,
                    e.type AS type_brand,
                    h.name AS grades
                    FROM
                    production_handovers a
                    ...
                    LEFT JOIN patterns f ON f.id = d.pattern_id
                    LEFT JOIN types g ON g.id = d.type_id
                    LEFT JOIN grades h ON h.id = d.grade_id
                    LEFT JOIN item_shadings k ON k.id = b.item_shading_id
                    WHERE
                    a.void_date IS NULL
                    AND a.deleted_at IS NULL
                    AND d.item_group_id = 7
                ...
                LEFT JOIN (
                    ...
                    (
                        ...
                        k.code AS shading,
                        coalesce(
                            SUM(b.qty * c.conversion),
                            0
                        ) AS Qty
                        FROM
                        production_handovers a
                        LEFT JOIN production_handover_details b ON a.id = b.production_handover_id
                        LEFT JOIN production_fg_receive_details c ON c.id = b.production_fg_receive_detail_id
                        ...
                        AND a.deleted_at IS NULL
                        AND d.item_group_id = 7
                        AND a.post_date < '".$this->start_date."'
                        GROUP BY
                        d.code,
                        d.name,
                        k.code
                        ...
                        k.code,
                        coalesce(
                            SUM(b.qty * f.qty_conversion),
                            0
                        )*-1 AS qtySJ
                        FROM
                        marketing_order_delivery_processes a
                ...
                LEFT JOIN (
                    ...
                    AND d.item_group_id = 7
                    AND a.post_date >= '".$this->start_date."'
                    AND a.post_date <= '".$this->finish_date."'
                    GROUP BY
                    d.code,
                    d.name,
                    k.code
                ) c ON c.code = a.code
                AND c.shading = a.shading
                LEFT JOIN (
                    SELECT
                    d.code,
                    d.name,
                ...
                LEFT JOIN (
                    ...
                    and a.id not in (
                        ...
                        from
                        marketing_order_delivery_process_tracks
                        where
                        status = '2'
                        and created_at <= '".$this->finish_date." 23:59:59'
