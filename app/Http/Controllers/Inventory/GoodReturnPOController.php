...
class GoodReturnPOController extends Controller
{
    ...
    public function datatable(Request $request){
                ...
                if($search) {
                    $query->where(function($query) use ($search, $request) {
                        $query->where('code', 'like', "%$search%")
                            ...
                            ->orWhereHas('goodReturnPODetail',function($query) use($search, $request){
        ...
        $total_filtered = GoodReturnPO::where(function($query) use ($search, $request) {
                if($search) {
                    $query->where(function($query) use ($search, $request) {
                        $query->where('code', 'like', "%$search%")
                            ->orWhere('post_date', 'like', "%$search%")
    ...
    public function getGoodReceipt(Request $request){
        $data = GoodReceipt::where('id',$request->id)->whereIn('status',['2','3'])->first();
   ...
   public function getCode(Request $request){
    ...
    public function create(Request $request){
        $validation = Validator::make($request->all(), [
           ...
           /*  'code.string'                       => 'Kode harus dalam bentuk string.',
            ...
            'arr_good_receipt_detail.required'  => 'Good Receipt detail tidak boleh kosong',
            'arr_good_receipt_detail.array'     => 'Good Receipt detail harus dalam bentuk array',
		]);
        ...
        if($validation->fails()) {
			...
			/* if($request->temp){
                ...
                try {
                    ...
                    if(in_array($query->status,['1','6'])){
                        if($request->has('file')) {
                            ...
                            $document = $request->file('file')->store('public/good_return_po');
                        } else {
                        ...
                        return response()->json([
                            'status'  => 500,
					        'message' => 'Status good return po / pengembalian po sudah diupdate dari menunggu, anda tidak bisa melakukan perubahan.'
                        ]);
                    }
                }catch(\Exception $e){
                    DB::rollback();
                ...
                try {
                    ...
                    $menu = Menu::where('url', $lastSegment)->first();
                    $newCode=GoodReturnPO::generateCode($menu->document_code.date('y',strtotime($request->post_date)).$request->code_place_id);
                    ...
                    $query = GoodReturnPO::create([
                        ...
                        'document'              => $request->file('document') ? $request->file('document')->store('public/good_return_po') : NULL,
                        'note'                  => $request->note,
                        'status'                => '1',
                        'total'                 => $totalall,
                        'tax'                   => $taxall,
			...
			if($query) {
                ...
                activity()
                    ...
                    ->log('Add / edit pengembalian barang po.');
    ...
    public function rowDetail(Request $request)
    {
        ...
        $string = '<div class="row pt-1 pb-1 lighten-4">
                        ...
                        <div class="col s12">
                            <table class="bordered" style="min-width:100%;max-width:100%;">
                                ...
                                <tbody>';
        $totalqtyditerima=0;
        $totalqtydikembalikan=0;
        foreach($data->goodReturnPODetail as $key => $rowdetail){
            $totalqtyditerima+=$rowdetail->goodReceiptDetail->qty;
            $totalqtydikembalikan+=$rowdetail->qty;
            $string .= '<tr>
        ...
        $string .= '<tr>
                ...
                <td class="right-align" style="font-weight: bold; font-size: 16px;">' . number_format($totalqtyditerima, 3, ',', '.') . '</td>
                <td class="right-align" style="font-weight: bold; font-size: 16px;">' . number_format($totalqtydikembalikan, 3, ',', '.') . '</td>
            </tr>  
        ';
        ...
        if($validation->fails()) {
            ...
            foreach($request->arr_id as $key =>$row){
                ...
                if($pr){
                    ...
                    $pdf = PrintHelper::print($pr,'Good Return','a5','landscape','admin.print.inventory.good_return_po_individual');
            ...
            if($validation->fails()) {
                ...
                elseif($total_pdf>31){
                    ...
                    for ($nomor = intval($request->range_start); $nomor <= intval($request->range_end); $nomor++) {
                        ...
                        $query = GoodReturnPO::where('Code', 'LIKE', '%'.$x)->first();
                        if($query){
                            $pdf = PrintHelper::print($query,'Good Return','a5','landscape','admin.print.inventory.good_return_po_individual');
                            $font = $pdf->getFontMetrics()->get_font("helvetica", "bold");
    ...
    public function printIndividual(Request $request,$id){
        ...
        $formattedDate = $currentDateTime->format('d/m/Y H:i:s');        
        if($pr){
            $pdf = PrintHelper::print($pr,'Good Return','a5','landscape','admin.print.inventory.good_return_po_individual',$menuUser->mode);
    ...
    public function exportFromTransactionPage(Request $request){
		...
		return Excel::download(new ExportGoodReturnPOTransactionPage($search,$post_date,$end_date,$status,$modedata), 'purchase_request_'.uniqid().'.xlsx');
    }
    ...
    public function viewStructureTree(Request $request){
        ...
        if($query) {
            ...
            $data_go_chart[]=$data_good_return;
            $data_id_greturns[]=$query->id;
            

            //pengambilan foreign branch
            
            $result = TreeHelper::treeLoop1($data_go_chart,$data_link,'data_id_greturns',$query->id);
            $array1 = $result[0];
            $array2 = $result[1];
    ...
    public function viewJournal(Request $request,$id){
        ...
        $query = GoodReturnPO::where('code',CustomHelper::decrypt($id))->first();
