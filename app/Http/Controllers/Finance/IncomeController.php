...
class IncomeController extends Controller
{
   ...
   public function getCode(Request $request){
        $menu = Menu::where('url', 'incomes')->first();
        UsedData::where('user_id', session('bo_id'))->delete();
        $code = Income::generateCode($menu->document_code.date('y',strtotime(now())));
    ...
    public function datatable(Request $request){
        ...
        $total_data = Income::count();

        $query_data = Income::where(function($query) use ($search, $request) {
                if($search) {
                    $query->where(function($query) use ($search, $request) {
                        $query->where('code', 'like', "%$search%")
                            ...
                            ->orWhereHas('IncomeDetail',function($query) use($search, $request){
        ...
        $total_filtered = Income::where(function($query) use ($search, $request) {
                if($search) {
                    $query->where(function($query) use ($search, $request) {
                        $query->where('code', 'like', "%$search%")
                            ->orWhere('post_date', 'like', "%$search%")
    ...
    public function create(Request $request){
        ...
        if($validation->fails()) {
			...
			if($request->temp){
                DB::beginTransaction();
                try {
                    $query = Income::where('code',CustomHelper::decrypt($request->temp))->first();
                    ...
                    if($request->has('file')) {
                        ...
                        {
                            $arrFile[] = $file->store('public/income');
                        }
                    ...
                    $query->save();
                    foreach($query->incomeDetail as $row){
                        $row->delete();
                    }
                ...
                try {
                    ...
                    $fileUpload='';
                    $menu = Menu::where('url', $lastSegment)->first();
                    $newCode=Income::generateCode($menu->document_code.date('y',strtotime($request->post_date)).$request->code_place_id);
			...
			if($query) {
                DB::beginTransaction();
                info($request);
                foreach($request->arr_note as $key => $row){
    ...
    public function show(Request $request){
        $po = Income::where('code',CustomHelper::decrypt($request->id))->first();
        $po['grandtotal'] = number_format($po->grandtotal,2,',','.');
        $arr=[];
        foreach($po->incomeDetail as $row){
            $arr[] = [
                'note'              => $row->note ?? '',
