...
class ResetCogsHelper
{
    public static function resetCogsWithoutJob(string $date = null, int $company_id = null, int $place_id = null, int $item_id = null, int $area_id = null, int $item_shading_id = null, int $production_batch_id = null, string $end_date = null){
		...
		if($bomPowder && $bomGroup == '1'){
			$itemcogs = ItemCogs::where('date','>=',$date)->where('date','<=',$end_date)->where('company_id',$company_id)->where('place_id',$place_id)->where('item_id',$item_id)->orderBy('date')->delete();
		...
		foreach ($period as $key => $date) {
			...
			if(count($arrIssueGlaze) > 0){
				foreach($arrIssueGlaze as $id){
					...
					if($ig){
						$ig->updateGrandtotal();
						self::resetCogsWithoutJob($dateloop,$ig->company_id,$ig->place_id,$ig->itemStock->item_id,$ig->itemStock->area_id,$ig->itemStock->item_shading_id,$ig->itemStock->production_batch_id,$end_date);
					}
				}   
			}
			...
			if(count($arrProductionFgReceive) > 0){
				foreach($arrProductionFgReceive as $row){
					...
					$pfr->recalculate();
					/* self::resetCogsWithoutJob($dateloop,$pfr->company_id,$pfr->place_id,$pfr->productionOrderDetail->productionScheduleDetail->item_id,NULL,NULL,NULL,date('Y-m-d')); */
				}
			}
			...
			foreach($productionhandoverout as $row){
				...
				$totalBefore = $total_final;
				$qtyBefore = $qty_final;
				/* self::resetCogsWithoutJob($dateloop,$row->productionHandover->company_id,$row->productionHandover->productionFgReceive->place_id,$row->item_id,$row->area_id,$row->item_shading_id,$row->productionBatch->id,date('Y-m-d')); */
			}
			...
			foreach($productionrepack as $row){
				...
				self::resetCogsWithoutJob($dateloop,$row->productionRepack->company_id,$row->place_id,$row->item_target_id,$row->area_id,$row->item_shading_id,$row->production_batch_id,$end_date);
				$qtyBefore = $qty_final;
				$totalBefore = $total_final;
			}
