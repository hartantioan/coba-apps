...
class ProductionBatchStockController extends Controller
{
    ...
    public function filter(Request $request){
        ...
        $perlu = 0 ;
        $query_data = ItemCogs::whereRaw("id IN (SELECT MAX(id) FROM item_cogs WHERE deleted_at IS NULL AND date <= '".$request->finish_date."' GROUP BY item_id, production_batch_id)")
