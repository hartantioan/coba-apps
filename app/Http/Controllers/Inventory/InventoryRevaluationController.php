...
class InventoryRevaluationController extends Controller
{
   ...
   public function getCode(Request $request){
    ...
    public function show(Request $request){
        ...
        foreach($ir->inventoryRevaluationDetail as $row){
            $arr[] = [
                ...
                'qty_new'           => CustomHelper::formatConditionalQty($row->itemStock->qty),
        ...
        if($validation->fails()) {
            ...
            foreach($request->arr_id as $key =>$row){
                ...
                if($pr){
                    ...
                    $pdf = PrintHelper::print($pr,'Revaluasi Inventori','a5','landscape','admin.print.inventory.revaluation_individual');
            ...
            if($validation->fails()) {
                ...
                elseif($total_pdf>31){
                    ...
                    for ($nomor = intval($request->range_start); $nomor <= intval($request->range_end); $nomor++) {
                        ...
                        if($query){
                            $pdf = PrintHelper::print($query,'Revaluasi Inventori','a5','landscape','admin.print.inventory.revaluation_individual');
                            $font = $pdf->getFontMetrics()->get_font("helvetica", "bold");
    ...
    public function printIndividual(Request $request,$id){
        ...
        $formattedDate = $currentDateTime->format('d/m/Y H:i:s');        
        if($pr){
            $pdf = PrintHelper::print($pr,'Revaluasi Inventori','a5','landscape','admin.print.inventory.revaluation_individual',$menuUser->mode);
    ...
    public function done(Request $request){
        $query_done = InventoryRevaluation::where('code',CustomHelper::decrypt($request->id))->first();
        ...
        if($query_done){
            ...
            if(in_array($query_done->status,['1','2'])){
                ...
                activity()
                        ...
                        ->log('Done the Inventory Revaluation data');
