...
class AssetGroupController extends Controller
{
    public function index()
    {
        $data = [
            'title'     => 'Grup Aset',
            'content'   => 'admin.master_data.asset_group',
            'parent'    => AssetGroup::where('status','1')->get(),
    ...
    public function datatable(Request $request){
        ...
        $total_data = AssetGroup::count();
        
        $query_data = AssetGroup::where(function($query) use ($search, $request) {
                if($search) {
        ...
        $total_filtered = AssetGroup::where(function($query) use ($search, $request) {
                if($search) {
                    $query->where(function($query) use ($search, $request) {
                        $query->where('code', 'like', "%$search%")
                            ->orWhere('name', 'like', "%$search%");
    ...
    public function create(Request $request){
        ...
        if($validation->fails()) {
			...
			if($request->temp){
                DB::beginTransaction();
                try {
                    $query = AssetGroup::find($request->temp);
			...
			if($query) {
                ...
                foreach(AssetGroup::whereNull('parent_id')->get() as $m){
    ...
    public function destroy(Request $request){
        $query = AssetGroup::find($request->id);
    ...
    public function print(Request $request){
        ...
        if($validation->fails()) {
            ...
            foreach($request->arr_id as $key =>$row){
                $pr[]= AssetGroup::where('code',$row)->first();
            ...
            $data["image"]=$path_img;
            $pdf = Pdf::loadView('admin.print.master_data.asset_group', $data)->setPaper('a5', 'landscape');
            $pdf->render();
