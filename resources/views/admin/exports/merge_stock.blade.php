<table border="1" cellpadding="2" cellspacing="0" style="width:100%; font-size:13px;border-collapse: collapse;">
    <thead>
        <tr align="center">
            <th>{{ __('translations.no') }}.</th>
            <th>No. Dokumen</th>
            <th>{{ __('translations.status') }}</th>
            <th>Voider</th>
            <th>Tgl. Void</th>
            <th>Ket. Void</th>
            <th>Deleter</th>
            <th>Tgl. Delete</th>
            <th>Ket. Delete</th>
            <th>Doner</th>
            <th>Tgl. Done</th>
            <th>Ket. Done</th>
            <th>NIK</th>
            <th>{{ __('translations.user') }}</th>
            <th>{{ __('translations.post_date') }}</th>
            <th>Item</th>
            <th>Plant Asal</th>
            <th>Warehouse Asal</th>
            <th>Area Asal</th>
            <th>Batch Tujuan</th>
            <th>Shading</th>
            <th>Keterangan</th>
            <th>Kode Item Bahan</th>
            <th>Nama Item Bahan</th>
            <th>Batch Bahan</th>
            <th>Shading Bahan</th>
            <th>Area Bahan</th>
            <th>Plant Bahan</th>
            <th>Gudang Bahan</th>
            <th>Qty Bahan</th>
            <th>Satuan Bahan</th>
            <th>Nominal Bahan</th>
        </tr>
    </thead>
    <tbody>
        @php
            $no = 1;
        @endphp
        @foreach($data as $row)
            @foreach ($row->mergeStockDetail()->orderBy('id')->get() as $row_detail)
                <tr align="center">
                    <td>{{ $no }}</td>
                    <td>{{ $row->code }}</td>
                    <td>{!! $row->statusRaw() !!}</td>
                    <td>{{ $row->voidUser()->exists() ? $row->voidUser->name : '' }}</td>
                    <td>{{ $row->voidUser()->exists() ? date('d/m/Y',strtotime($row->void_date)) : '' }}</td>
                    <td>{{ $row->voidUser()->exists() ? $row->void_note : '' }}</td>
                    <td>{{ $row->deleteUser()->exists() ? $row->deleteUser->name : '' }}</td>
                    <td>{{ $row->deleteUser()->exists() ? date('d/m/Y',strtotime($row->deleted_at)) : '' }}</td>
                    <td>{{ $row->deleteUser()->exists() ? $row->delete_note : '' }}</td>
                    <td>{{($row->status == 3 && is_null($row->done_id)) ? 'sistem' : (($row->status == 3 && !is_null($row->done_id)) ? $row->doneUser->name : null)}}</td>
                    <td>{{ $row->doneUser ? $row->done_date : '' }}</td>
                    <td>{{ $row->doneUser ? $row->done_note : '' }}</td>
                    <td>{{ $row->user->employee_no }}</td>
                    <td>{{ $row->user->name }}</td>
                    <td>{{ date('d/m/Y',strtotime($row->post_date)) }}</td>
                    <td>{{ $row->item->code.' - '.$row->item->name }}</td>
                    <td>{{ $row->itemStock->place->code }}</td>
                    <td>{{ $row->itemStock->warehouse->name }}</td>
                    <td>{{ $row->itemStock->area->code }}</td>
                    <td>{{ $row->batch_no }}</td>
                    <td>{{ $row->itemStock->itemShading->code }}</td>
                    <td>{{ $row->note }}</td>
                    <td>{{ $row_detail->item->code }}</td>
                    <td>{{ $row_detail->item->name }}</td>
                    <td>{{ $row_detail->itemStock->productionBatch->code }}</td>
                    <td>{{ $row_detail->itemStock->itemShading->code }}</td>
                    <td>{{ $row_detail->itemStock->area->name }}</td>
                    <td>{{ $row_detail->itemStock->place->code }}</td>
                    <td>{{ $row_detail->itemStock->warehouse->name }}</td>
                    <td>{{ CustomHelper::formatConditionalQty(round($row_detail->qty,2))}}</td>
                    <td>{{ $row_detail->item->uomUnit->code }}</td>
                    <td>{{ $nominal ? number_format(round($row_detail->total,2) ,3,',','.') : '-' }}</td>

                </tr>
                @php
                    $no++;
                @endphp
            @endforeach

        @endforeach
    </tbody>
</table>
