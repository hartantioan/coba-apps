    ...
    public function onRow(Row $row)
    {
        ...
        try {
            if (isset($row['code']) && $row['code']) {
                ...
                if(isset($row['code']) && $row['code']){
                    $check = DeliveryCost::where('code',$row['code'])->first();
                ...
                if($check){
                    ...
                    $check->name = $row['name'];
                    $check->valid_from = $dateFormatted1;
                    $check->valid_to = $dateFormatted2;
                    $check->from_city_id = $city_id_from;
                    $check->from_subdistrict_id = $sub_district_id_from;
                    $check->to_city_id = $city_id_to;
                    $check->to_subdistrict_id = $sub_district_id_to;
                    $check->transportation_id = $transport->id;
                    $check->tonnage = $tonnage;
                    $check->ritage = $ritage;
                    $check->qty_tonnage = $tonnage_weight;
                    $check->status =  1;
