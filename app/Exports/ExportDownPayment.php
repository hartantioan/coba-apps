    ...
    public function view(): View
    {
        ...
        $query_data = DB::select("
            ...
            foreach($query_data as $row_invoice){
                if($this->date >= '2024-11-30'){
                    $currency_rate = $row_invoice->latest_currency_before > 0 ? $row_invoice->latest_currency_before : $row_invoice->currency_rate;
                    $total_adjust_new_rule = 0;
                    if($row_invoice->latest_reverse_date >= '2024-11-01' && $row_invoice->post_date >= '2024-10-01'){
                        $total_adjust_new_rule = round(($row_invoice->total_used / $row_invoice->grandtotal) * $row_invoice->adjust_nominal_latest,2);
                    }
                    $total_received_after_adjust = round($row_invoice->grandtotal * $currency_rate, 2) + $row_invoice->adjust_nominal_after_update;
