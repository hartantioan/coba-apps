...
class ApprovalStageController extends Controller
{
    ...
    public function show(Request $request){
        ...
        foreach($approval->approvalStageDetail as $row){
            $details[] = [
                'user_id'   => $row->user_id,
                'user_name' => $row->user->employee_no.' - '.$row->user->name.' - '.$row->user->phone.' Pos. '.($row->user->position()->exists() ? $row->user->position->Level->name.' Div. '.$row->user->position->division->name : 'N/A'),
            ];
        }
