    ...
    public function datatable(Request $request){
        ...
        if($query_data <> FALSE) {
            ...
            foreach($query_data as $val) {
                ...
                if($request->modedata){
                    $voidbutton = '<button type="button" class="btn-floating mb-1 btn-flat waves-effect waves-light amber accent-2 white-tex btn-small" data-popup="tooltip" title="Tutup" '.$dis.' onclick="voidStatus(`' . CustomHelper::encrypt($val->code) . '`)"><i class="material-icons dp48">close</i></button>';
                }
    ...
    public function show(Request $request){
        ...
        foreach($po->purchaseOrderDetail as $row){
            $arr[] = [
                ...
                'qty'                               => CustomHelper::formatConditionalQty($row->qty),
                'qty_limit'                         => $row->purchaseRequestDetail()->exists() ? CustomHelper::formatConditionalQty($row->purchaseRequestDetail->qtyBalance() + $row->qty) : CustomHelper::formatConditionalQty($row->qty),
                ...
                'total'                             => CustomHelper::formatConditionalQty($row->total ?? ($row->subtotal ?? 0)),
            ...
            if($validation->fails()) {
                ...
                elseif($total_pdf>31){
                    ...
                    for ($nomor = intval($request->range_start); $nomor <= intval($request->range_end); $nomor++) {
                        ...
                        if($query){
                            $pdf = PrintHelper::print($query,'Print Purchase Order','a4','portrait','admin.print.purchase.order_individual');
                            $font = $pdf->getFontMetrics()->get_font("helvetica", "bold");
            ...
            if($validation->fails()) {
                ...
                if(count($merged)>31){
                    ...
                    foreach($merged as $code){
                        ...
                        if($query){
                            $pdf = PrintHelper::print($query,'Purchase Order','a4','portrait','admin.print.purchase.order_individual');
                            $font = $pdf->getFontMetrics()->get_font("helvetica", "bold");
                            $pdf->getCanvas()->page_text(495, 785, "Jumlah Print, ". $query->printCounter()->count(), $font, 10, array(0,0,0));
                            $pdf->getCanvas()->page_text(505, 800, "PAGE: {PAGE_NUM} of {PAGE_COUNT}", $font, 10, array(0,0,0));
                            $pdf->getCanvas()->page_text(422, 810, "Print Date ". $formattedDate, $font, 10, array(0,0,0));
