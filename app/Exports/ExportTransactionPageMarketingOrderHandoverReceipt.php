...
class ExportTransactionPageMarketingOrderHandoverReceipt implements FromCollection, WithTitle, WithHeadings, ShouldAutoSize
{
    ...
    {
        $query_data = MarketingOrderHandoverReceipt::where(function($query)  {
                if($this->search) {
        ...
        foreach($query_data as $key => $row){
            ...
            $arr[] = [
                ...
                'collector'        => $row->account->name,
                'perusahaan'           => $row->company->name,
                'total_tagihan'         =>$row->grandtotal,
                'total_diterima'        => $row->totalReceived(),
                'total_kembali'           => $row->totalUnreceived(),
    ...
    {
        return 'Marketing Order Handover Receipt';
    }
