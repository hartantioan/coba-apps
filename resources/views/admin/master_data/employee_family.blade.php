                ...
                <form class="row" id="form_data" onsubmit="return false;">
                    ...
                    <div class="col s12">
                        <div class="input-field col s12 m6">
                            ...
                            <label class="name" for="name">{{ __('translations.name') }}</label>
                        </div>
                        <div class="input-field col s12 m6">
                            <select class="form-control" id="relation" name="relation">
                                ...
                                <option value="7">Sepupu</option>
                                <option value="8">Paman</option>
                                <option value="9">Bibi</option>
                                <option value="10">Others</option>
                                <option value="11">Adik</option>
                                <option value="12">Kakak</option>
                            </select>
                        ...
                        <div class="input-field col s12 m6">
                            ...
                            <label class="address" for="address">{{ __('translations.address') }}</label>
                        </div>
                        <div class="input-field col s12 m6">
                        ...
                        <div class="input-field col s12 m6">
                            ...
                            <label for="marriage_status">{{ __('translations.status') }}</label>
                        </div>
                        <div class="input-field col s12 m6">
...
document.addEventListener('focusin', function (event) {
    ...
    function save(){
        ...
        $.ajax({
            url: '{{ URL::to('/') }}/admin/master_data/master_hr/employee/create_family',
            type: 'POST',
            dataType: 'JSON',
            data: formData,
            contentType: false,
    ...
    function loadDataTable() {
		...
		window.table = $('#datatable_serverside').DataTable({
            ...
            "order": [[0, 'desc']],
            ajax: {
                url: '{{ URL::to('/') }}/admin/master_data/master_hr/employee/datatable_family',
                type: 'GET',
                data: {
            ...
            columns: [
                ...
                { name: 'relation', className: 'center-align' },
                { name: 'emergency_contact', className: 'center-align' },
                { name: 'address', className: 'center-align' },
                { name: 'id_number', className: 'center-align' },
                { name: 'marriage_status', className: 'center-align' },
                { name: 'religion', className: 'center-align' },
                { name: 'job', className: 'center-align' },
                { name: 'birth_date', className: 'center-align' },
    ...
    function show(id){
        $.ajax({
            url: '{{ URL::to('/') }}/admin/master_data/master_hr/employee/show_family',
            type: 'POST',
            dataType: 'JSON',
            data: {
                id: id
            ...
            success: function(response) {
                loadingClose('#main');
                $('#modal1').modal('open');
                console.log(response.marriage_status);
                $('#temp').val(id);
                $('#relation').val(response.relation).formSelect();
                $('#name').val(response.name);
                $('#address').val(response.address);
                $('#emergency_contact').val(response.emergency_contact);
                $("#religion").val(response.religion).formSelect();
                $("#marriage_status").val(response.marriage_status).formSelect();
    ...
    function destroy(id){
        swal({
            ...
            if (willDelete) {
                $.ajax({
                    url: '{{ URL::to('/') }}/admin/master_data/master_hr/employee/destroy_family',
                    type: 'POST',
                    dataType: 'JSON',
                    data: { id : id },
                    headers: {
