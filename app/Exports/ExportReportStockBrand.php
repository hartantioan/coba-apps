...
class ExportReportStockBrand implements FromCollection, WithTitle, WithHeadings, ShouldAutoSize
{
    ...
    public function collection()
    {
        ...
        //ambil brandid sesuai bo_id
        $userbrand = UserBrand::where('account_id',session('bo_id'))->get('brand_id');

        $item = ItemShading::join('items', 'item_shadings.item_id', '=', 'items.id')->whereIn('brand_id',$userbrand)
        ->orderBy('items.code')
        ->orderBy('items.id')
