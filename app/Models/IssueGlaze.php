...
class IssueGlaze extends Model
{
    ...
    public static function generateCode($prefix)
    {
        ...
        $query = IssueGlaze::selectRaw('RIGHT(code, 8) as code')
    ...
    {
        return $this->hasMany('App\Models\ReceiveGlazeDetail')->whereHas('receiveGlaze',function($query){
    ...
    public function hasChildDocument(){
        ...
        if($this->receiveGlazeDetail()->exists()){
            $hasRelation = true;
        }
    ...
    public function updateGrandtotal(){
        $total = $this->issueGlazeDetail()->sum('total');
        $this->update([
            'grandtotal'    => $total
        ]);
    }
    ...
    public function balance(){
        $total = $this->qty - $this->receiveGlazeDetail()->sum('qty');
        return $total;
    }
