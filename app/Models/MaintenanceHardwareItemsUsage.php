...
class MaintenanceHardwareItemsUsage extends Model
{
    use HasFactory, SoftDeletes, Notifiable;

    protected $table = 'maintenance_hardware_items_usages';
    ...
    public function requestRepairHardwareItemUsage(){
        return $this->belongsTo('App\Models\RequestRepairHardwareItemsUsage', 'request_repair_hardware_items_usage_id', 'id')->withTrashed();
    }

    public function attachmentMaintenanceHardwareItemsUsage(){
        return $this->hasMany('App\Models\AttachmentMaintenanceHardwareItemsUsage');
    }

    public static function generateCode()
    {
        $query = MaintenanceHardwareItemsUsage::selectRaw('RIGHT(code, 6) as code')
            ->withTrashed()
            ->orderByDesc('id')
            ->limit(1)
            ->get();
