...
class ExportInventoryIssueDetail implements FromCollection, WithTitle, WithHeadings, ShouldAutoSize
{
    ...
    {
        $query_data = InventoryIssueDetail::whereHas('inventoryIssue', function ($query) {
                if($this->search) {
        ...
        foreach($query_data as $key => $row){
            ...
            $arr[] = [
                'no'          => $key + 1,
                'status'      => $row->inventoryIssue->statusRaw() ?? '',
                'grandtotal'  => number_format($row->inventoryIssue->grandtotal, 2, ',', '.'),
                'kode'        => $row->inventoryIssue->code ?? '',
                'tgl_posting' => $row->inventoryIssue->post_date ? date('d/m/Y', strtotime($row->inventoryIssue->post_date)) : '',
                'item'        => $row->itemStockNew->item->name ?? '',
                'qty'         => number_format($row->qty, 2, ',', '.'),
                'harga'       => number_format($row->price, 2, ',', '.'),
                'keterangan'  => $row->note ?? '',
                'item_store'  => $row->storeItemStock->item->name ?? '',
            ];
