...
use App\Exports\ExportGoodReturnIssue;
use App\Exports\ExportGoodReturnIssueTransactionPage;
...
class GoodReturnIssueController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = [
            ...
            'content'   => 'admin.inventory.good_return_issue',
   ...
   public function getCode(Request $request){
    ...
    public function datatable(Request $request){
        ...
        $total_data = GoodReturnIssue::whereRaw("SUBSTRING(code,8,2) IN ('".implode("','",$this->dataplacecode)."')")
        ...
        $query_data = GoodReturnIssue::where(function($query) use ($search, $request) {
                if($search) {
                    $query->where(function($query) use ($search, $request) {
                        $query->where('code', 'like', "%$search%")
                            ->orWhere('post_date', 'like', "%$search%")
                            ->orWhere('note', 'like', "%$search%")
                            ->orWhereHas('goodReturnIssueDetail', function($query) use($search, $request){
        ...
        $total_filtered = GoodReturnIssue::where(function($query) use ($search, $request) {
                if($search) {
                    $query->where(function($query) use ($search, $request) {
                        $query->where('code', 'like', "%$search%")
                            ->orWhere('post_date', 'like', "%$search%")
    ...
    public function create(Request $request){
        $validation = Validator::make($request->all(), [
           ...
           /*  'code.string'                       => 'Kode harus dalam bentuk string.',
            ...
            'arr_item.array'                    => 'Item stok harus dalam bentuk array',
            'arr_qty.required'                  => 'Qty item tidak boleh kosong',
            'arr_qty.array'                     => 'Qty item harus dalam bentuk array',
            'arr_good_issue_detail.required'    => 'Item Good Issue tidak boleh kosong',
            'arr_good_issue_detail.array'       => 'Item Good Issue harus dalam bentuk array',
		]);
        ...
        if($validation->fails()) {
            ...
            try {
                ...
                if($request->temp){
                    $query = GoodReturnIssue::where('code',CustomHelper::decrypt($request->temp))->first();
                    ...
                    if(in_array($query->status,['1','6'])){
                        if($request->has('file')) {
                            ...
                            $document = $request->file('file')->store('public/good_return_issues');
                        } else {
                        ...
                        foreach($query->goodReturnIssueDetail as $row){
                            $row->delete();
                        }
                    }else{
                        return response()->json([
                    ...
                    $query = GoodReturnIssue::create([
                        ...
                        'document'              => $request->file('document') ? $request->file('document')->store('public/good_return_issues') : NULL,
                        'note'                  => $request->note,
                        'status'                => '1',
                ...
                if($query) {
                    ...
                    foreach($request->arr_good_issue_detail as $key => $row){
                        $gid = GoodReturnIssueDetail::create([
                            'good_return_issue_id'          => $query->id,
                            'good_issue_detail_id'          => $row,
    ...
    public function rowDetail(Request $request){
                    ...
                    <table style="min-width:100%;max-width:100%;">
                        <thead>
                            ...
                            <tr>
                                ...
                                <th class="center-align" rowspan="2">Item</th>
                                <th class="center-align" colspan="5">Informasi Asal</th>
                                <th class="center-align" rowspan="2">Qty Keluar</th>
                                <th class="center-align" rowspan="2">Qty Kembali</th>
                                <th class="center-align" rowspan="2">Satuan</th>
        ...
        $totalqtykeluar=0;
        $totalqtykembali=0;
        foreach($data->goodReturnIssueDetail as $key => $row){
            $totalqtykeluar+=$row->goodIssueDetail->qtyBalanceReturn();
            $totalqtykembali+=$row->qty;
            $string .= '<tr>
                <td class="center-align">'.($key + 1).'</td>
                <td class="">'.$row->item->code.' - '.$row->item->name.'</td>
                <td class="">'.$row->goodIssueDetail->itemStock->place->code.'</td>
                <td class="">'.$row->goodIssueDetail->itemStock->warehouse->name.'</td>
        ...
        $string .= '<tr>
                <td class="center-align" style="font-weight: bold; font-size: 16px;" colspan="7"> Total </td>
                <td class="right-align" style="font-weight: bold; font-size: 16px;">' . number_format($totalqtykeluar, 3, ',', '.') . '</td>
                <td class="right-align" style="font-weight: bold; font-size: 16px;">' . number_format($totalqtykembali, 3, ',', '.') . '</td>
            </tr>  
        ';
    ...
    public function show(Request $request){
        $gr = GoodReturnIssue::where('code',CustomHelper::decrypt($request->id))->first();
        ...
        foreach($gr->goodReturnIssueDetail as $row){
            $arr[] = [
                'good_issue_id'         => $row->goodIssueDetail->goodIssue->id,
                'good_issue_detail_id'  => $row->good_issue_detail_id,
                'item_id'               => $row->item_id,
                'item_name'             => $row->item->code.' - '.$row->item->name,
                'place_name'            => $row->goodIssueDetail->itemStock->place->code,
    ...
    public function voidStatus(Request $request){
        $query = GoodReturnIssue::where('code',CustomHelper::decrypt($request->id))->first();
        ...
        if($query) {
            ...
            if(in_array($query->status,['4','5'])){
                ...
                activity()
                    ...
                    ->causedBy(session('bo_id'))
                    ->withProperties($query)
                    ->log('Void the good return issue data');
    
                CustomHelper::sendNotification($query->getTable(),$query->id,'Barang Kembali No. '.$query->code.' telah ditutup dengan alasan '.$request->msg.'.',$request->msg,$query->user_id);
    ...
    public function destroy(Request $request){
        ...
        if($query->delete()) {
            ...
            $query->goodReturnIssueDetail()->delete();
            ...
            activity()
                ...
                ->causedBy(session('bo_id'))
                ->withProperties($query)
                ->log('Delete the good return issue data');
    ...
    public function approval(Request $request,$id){
        ...
        $gr = GoodReturnIssue::where('code',CustomHelper::decrypt($id))->first();
        ...
        if($gr){
            ...
            return view('admin.approval.good_return_issue', $data);
        }else{
            abort(404);
        }
    }
    ...
    public function print(Request $request){
        ...
        if($validation->fails()) {
            ...
            foreach($request->arr_id as $key =>$row){
                $pr = GoodReturnIssue::where('code',$row)->first();
                
                if($pr){
                   
                    $pdf = PrintHelper::print($pr,'Barang Kembali','a5','landscape','admin.print.inventory.good_return_issue_individual');
            ...
            if($validation->fails()) {
                ...
                elseif($total_pdf>31){
                    ...
                    for ($nomor = intval($request->range_start); $nomor <= intval($request->range_end); $nomor++) {
                        ...
                        $query = GoodReturnIssue::where('Code', 'LIKE', '%'.$x)->first();
                        if($query){
                            $pdf = PrintHelper::print($query,'Barang Kembali','a5','landscape','admin.print.inventory.good_return_issue_individual');
                            $font = $pdf->getFontMetrics()->get_font("helvetica", "bold");
            ...
            if($validation->fails()) {
                ...
                if(count($merged)>31){
                    ...
                    foreach($merged as $code){
                        ...
                        $query = GoodReturnIssue::where('code', 'LIKE', '%'.$etNumbersArray[$code-1])->first();
                        if($query){
                            $pdf = PrintHelper::print($query,'Barang Kembali','a5','landscape','admin.print.inventory.good_return_issue_individual');
                            $font = $pdf->getFontMetrics()->get_font("helvetica", "bold");
                            $pdf->getCanvas()->page_text(495, 340, "Jumlah Print, ". $query->printCounter()->count(), $font, 10, array(0,0,0));
    ...
    public function printIndividual(Request $request,$id){
        ...
        $pr = GoodReturnIssue::where('code',CustomHelper::decrypt($id))->first();
        $currentDateTime = Date::now();
        $formattedDate = $currentDateTime->format('d/m/Y H:i:s');        
        if($pr){
            $pdf = PrintHelper::print($pr,'Barang Kembali','a5','landscape','admin.print.inventory.good_return_issue_individual',$menuUser->mode);
    ...
    public function export(Request $request){
		...
		return Excel::download(new ExportGoodReturnIssue($post_date,$end_date,$mode), 'good_return_issue_'.uniqid().'.xlsx');
    }
    public function exportFromTransactionPage(Request $request){
		...
		return Excel::download(new ExportGoodReturnIssueTransactionPage($search,$post_date,$end_date,$status,$modedata), 'purchase_request_'.uniqid().'.xlsx');
    }
    ...
    public function viewJournal(Request $request,$id){
        ...
        $query = GoodReturnIssue::where('code',CustomHelper::decrypt($id))->first();
    ...
    public function sendUsedData(Request $request){
        ...
        if(!$data->used()->exists()){
            CustomHelper::sendUsedData($request->type,$request->id,'Form Good Return Issue / Barang Kembali');
            return response()->json([
                'status'    => 200,
            ]);
        }else{
