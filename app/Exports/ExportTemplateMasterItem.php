...
class ExportTemplateMasterItem implements WithEvents
{
    ...
    {
        $event->writer->reopen(new \Maatwebsite\Excel\Files\LocalTemporaryFile(storage_path('app/public/format_imports/format_item_program.xlsx')),Excel::XLSX);
        $event->writer->getSheetByIndex(0); #main
        $event->writer->getSheetByIndex(1); #item group
        $event->writer->getSheetByIndex(2); #unit
        $event->writer->getSheetByIndex(3); #supplier
