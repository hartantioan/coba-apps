...
use App\Exports\ExportBomStandard;
use App\Exports\ExportTemplateMasterBomStandard;
use App\Imports\ImportBomStandard;
use App\Models\Line;
use App\Models\Machine;
use App\Models\Warehouse;
use Illuminate\Support\Str;
class BomStandardController extends Controller
{
    public function index()
    {
        $data = [
            'title'     => 'Bill of Material (Standard)',
            'content'   => 'admin.master_data.bom_standard',
    ...
    public function datatable(Request $request){
        ...
        $total_data = BomStandard::count();
        
        $query_data = BomStandard::where(function($query) use ($search, $request) {
                if($search) {
        ...
        $total_filtered = BomStandard::where(function($query) use ($search, $request) {
                if($search) {
                    $query->where(function($query) use ($search, $request) {
                        $query->where('code', 'like', "%$search%")
                            ->orWhere('name', 'like', "%$search%");
    ...
    public function create(Request $request){
        ...
        $validation = Validator::make($request->all(), [
            'code'              => $request->temp ? ['required', Rule::unique('bom_standards', 'code')->ignore($request->temp)] : 'required|unique:bom_standards,code',
            'name'                      => 'required',
        ...
        if($validation->fails()) {
			...
			if($request->temp){
                DB::beginTransaction();
                try {
                    $query = BomStandard::find($request->temp);
                    ...
                    $query->bomStandardDetail()->delete();
			...
			if($query) {               
                ...
                activity()
                    ...
                    ->log('Add / edit bom standard data.');
    ...
    {
        try {
            Excel::import(new ImportBomStandard, $request->file('file'));
    ...
    public function getImportExcel(){
        return Excel::download(new ExportTemplateMasterBomStandard(), 'format_master_bom_standard'.uniqid().'.xlsx');
    }

    public function rowDetail(Request $request)
    {
        $data   = BomStandard::where('code',CustomHelper::decrypt($request->id))->first();
        ...
        foreach($data->bomStandardDetail as $key => $m){
            $string .= '<tr>
    ...
    public function show(Request $request){
        $bom = BomStandard::find($request->id);

        $details = [];

        foreach($bom->bomStandardDetail as $m){
            $details[] = [
    ...
    public function destroy(Request $request){
        $query = BomStandard::find($request->id);
        ...
        if($query->delete()) {
            ...
            activity()
                ...
                ->causedBy(session('bo_id'))
                ->withProperties($query)
                ->log('Delete the bill of material standard data');
    ...
    public function print(Request $request){
        ...
        if($validation->fails()) {
            ...
            foreach($request->arr_id as $key =>$row){
                $pr[]= BomStandard::where('code',$row)->first();
