...
class MarketingOrderDeliveryProcessController extends Controller
{
    ...
    public function getStockByBarcode(Request $request){
        if($request->code){
            ...
            if($data){
                $qtyStock = round($data->balanceWithUnsent() / round($request->conversion,3),3);
                $qtyNeeded = 1;
        ...
        if($query_data <> FALSE) {
            ...
            foreach($query_data as $val) {
                ...
                $response['data'][] = [
                    ...
                    $val->marketingOrderDelivery->deliveryType(),
                    ...
                    if ($sama->isNotEmpty()) {
                        ...
                        if($query->marketingOrderDelivery->customer->mitraCustomer()->exists()){
                            ...
                            $payload=[
                                ...
                                "customerCode"   => optional($query->marketingOrderDelivery->customer->mitraCustomer)->code,
                                "salesOrderCode" => optional(($query->getSalesModelUnique()[0] ?? null)->mitraMarketingOrder)->document_no,
                                "details"        => $detailItem,
                            ];
                            // Log::info($payload);
    ...
    public function voidStatus(Request $request){
        ...
        if($query) {
            ...
            /* }elseif($query->marketingOrderDelivery()->exists() && $query->marketingOrderDelivery->goodScaleDetail()->exists() && $query->marketingOrderDelivery->goodScaleDetail->goodScale->time_scale_out){
                $response = [
                    'status'  => 500,
