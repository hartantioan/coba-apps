    ...
    <div class="card-content invoice-print-area">
        ...
        <div class="row invoice-date-number">
            <div class="col xl4 s5">
                <span class="invoice-number mr-1">Request Sparepart # {{ $data->code }}</span>
            </div>
        ...
        <div class="row mt-3 invoice-logo-title">
            <div class="col m6 s12">
                <h5 class="indigo-text">Request Sparepart</h5>
            </div>
                ...
                <tbody>
                    @foreach($data->requestSparePartDetail as $key => $row)
                        <tr>
                            ...
                            <td>{{ $row->equipmentSparepart->item->name }}</td>
                            <td class="center-align">{{$row->qty_request}}</td>
                            <td class="center-align">{{$row->qty_usage }}</td>
                            <td class="right-align">{{ $row->qty_return }}</td>
                            <td class="right-align">{{ $row->qty_repair }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <div class="invoice-subtotal mt-3">
            <div class="row">
                <div class="col m6 s6 l6">
                    {!! ucwords(strtolower($data->company->city->name)).', '.CustomHelper::tgl_indo($data->request_date) !!}
                </div>
                <div class="col m6 s6 l6">
                    Catatan : {{ $data->summary_issue }}
                </div>
            </div>
