...
class ExportMarketingRecapitulationCsv implements FromCollection, WithTitle, ShouldAutoSize
{
    ...
    public function collection()
    {
        ...
        foreach ($ardp as $key => $row) {
            ...
            $arr[] = [
                '1'     => 'FK;' . $transactionCode . ';' . $revCode . ';' . $tax_no . ';' . $month . ';' . $year . ';' . $newdate . ';' . $row->getNpwp() . ';' . $row->account->userDataDefault()->title . ';' . $row->account->userDataDefault()->address . ';' . round($row->total * (11/12), 0) . ';' . round($row->tax, 0) . ';0;;1;' . floor($row->total * (11/12)) . ';' . floor($row->tax) . ';0;' . $row->code . ';;'
            ];
            $arr[] = [
        ...
        foreach ($invoice as $key => $row) {
            ...
            if ($row->total > 0) {
                ...
                $arr[] = [
                    '1'     => 'FK;' . $transactionCode . ';' . $revCode . ';' . $tax_no . ';' . $month . ';' . $year . ';' . $newdate . ';' . $row->getNpwp() . ';' . $row->userData->title . ';' . $row->userData->address . ';' . $row->totalDpp2025() . ';' . $row->totalTax2025() . ';0;' . $freeAreaTax . ';2;0;0;0;' . $row->code . ';' . ($row->no_pjb ?? '') . ';'
                ];
