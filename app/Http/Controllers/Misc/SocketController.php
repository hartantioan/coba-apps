...
class SocketController extends Controller implements MessageComponentInterface
{
    ...
    public function onMessage(ConnectionInterface $conn, $msg)
    {
        ...
        if(isset($data->type))
        {
            ...
            if($data->type == 'request_search_connected_user')
            {
                ...
                foreach($user_id_data as $user_id_row)
                {
                    ...
                    $last_chat = Chat::select('from_user_id', 'to_user_id','chat_message','id')
                    ->where(function ($query) use ($user_id_row) {
                        $query->where('from_user_id', $user_id_row->from_user_id)
                            ->where('to_user_id', $user_id_row->to_user_id);
                    })
                    ->orWhere(function ($query) use ($user_id_row) {
                        $query->where('from_user_id', $user_id_row->to_user_id)
                            ->where('to_user_id', $user_id_row->from_user_id);
                    })
                    ...
                    if($user_data){
                        ...
                        $sub_data[] = array(
                            ...
                            'id_message'    =>  $last_chat->from_user_id.$last_chat->to_user_id,
                        );
                    }
                ...
                foreach($this->clients as $client)
                {
                    if($client->resourceId == $sender_connection_id[0]->connection_id)
                    {
                        $send_data['response_search_connected_user'] = true;
            ...
            if($data->type == 'request_load_unread_notification')
            {
                ...
                {
                    if($row->to_user_id == $data->user_id && $row->status == 'Pending'){
                        $count_notif_user++;
                    }
                    $user_id = '';
                ...
                foreach($this->clients as $client)
                {
                    if($client->resourceId == $sender_connection_id[0]->connection_id)
                    {
                        ...
                        $send_data['count_notification'] = $count_notif_user;
            ...
            if($data->type == 'request_process_chat_request')
            {
                ...
                if($data->action == 'Approve'){
                    ...
                    Chat_request::where('id', '!=', $chatRequestId)
                    ->where(function ($query) use ($fromUserId, $toUserId) {
                        $query->where(function ($query) use ($fromUserId, $toUserId) {
            ...
            if($data->type == 'request_connected_chat_user')
            {
                ...
                foreach($user_id_data as $user_id_row)
                {
                    ...
                    $last_chat = Chat::select('from_user_id', 'to_user_id','chat_message','id','created_at')
                    ->where(function ($query) use ($user_id_row) {
                        $query->where('from_user_id', $user_id_row->from_user_id)
                            ->where('to_user_id', $user_id_row->to_user_id);
                    })
                    ...
                    if($last_chat){
                        $sub_data[] = array(
                            ...
                            'last_seen'     =>  $last_seen,
                            'last_chat'     =>  $last_chat->chat_message,
                            'time'          =>  $last_chat->created_at,
                            'id_message'    =>  $user_id_row->from_user_id.$user_id_row->to_user_id,
                        );
                    }else{
                        $sub_data[] = array(
                            'id'    =>  $user_data->id,
            ...
            if($data->type == 'request_send_message')
            {
                ...
                foreach($this->clients as $client)
                {
                    if($client->resourceId == $receiver_connection_id[0]->connection_id || $client->resourceId == $sender_connection_id[0]->connection_id)
                    {
                        ...
                        $send_data['id_request_chat'] = $user_id_data->from_user_id.$user_id_data->to_user_id;
    ...
    public function onClose(ConnectionInterface $conn)
    {
        ...
        if(isset($queryarray['token']))
        {
            ...
            {
                $data['last_seen'] = '' . date('H:i');
            }
            else
            {
                $data['last_seen'] = '' . date('d/m/Y H:i');
            }
